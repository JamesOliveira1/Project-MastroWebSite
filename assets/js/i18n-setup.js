...
function updateFlags(lng) {
  ...
  if (container) {
    ...
    items.forEach(item => {
      // Verifica se o item corresponde ao idioma atual via data-label
      const itemLabel = item.getAttribute('data-label');
      if (itemLabel === labelMap[cleanLng]) {
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  i18next
    ...
    .init({
      ...
      // Tenta atualizar flags com persistência para garantir que o header esteja acessível
      const tryUpdateFlags = (attempts) => {
        if (attempts <= 0) return;
        updateFlags(i18next.language);
        setTimeout(() => tryUpdateFlags(attempts - 1), 100);
      };
      tryUpdateFlags(3);
  ...
  window.addEventListener('progenese:header-ready', function() {
    // Só atualiza conteúdo se o i18next já estiver totalmente inicializado (traduções carregadas)
    // Caso contrário, esperamos o callback do init() ou o evento i18next-ready fazerem isso.
    if (window.i18next && i18next.isInitialized) {
        updateContent();
        // Pequeno delay e múltiplas tentativas para garantir que o DOM do header foi renderizado
        // e que as bandeiras podem ser ocultadas corretamente
        const tryUpdateFlags = (attempts) => {
            if (attempts <= 0) return;
            updateFlags(i18next.language);
            setTimeout(() => tryUpdateFlags(attempts - 1), 100);
