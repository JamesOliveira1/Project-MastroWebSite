(() => {
  ...
  // Se NÃO estiver na página de login, segura o preloader para evitar flash de conteúdo
  if (!onLoginPage()) {
  ...
  function validateFields(identity, senha) {
    // Não bloqueia por complexidade da senha aqui; permite checar usuário no servidor
    if (!identity || !senha) return 'Preencha usuário ou e-mail e senha';
    if (isEmail(identity)) {
      // Validação simples de e-mail; validação definitiva no servidor
      return null;
    }
    if (identity.length < 3 || identity.length > 64) return 'Usuário inválido';
    return null;
  }
  ...
  async function handleLoginSubmit(ev) {
    ...
    try {
      ...
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify(isEmail(usuario)
          ? { action: 'login', email: usuario, senha, remember }
          : { action: 'login', usuario, senha, remember }
        )
      });
      const data = await res.json();
      if (!data || !data.ok) {
        ...
        const redirect = data.redirect || '/progenese/inicio.html';
  ...
  async function ensureAuthenticated() {
    try {
      ...
      if (!data || !data.ok || !data.authenticated) {
        window.location.replace(`${BASE_PREFIX}/progenese/login.html`);
      } else {
        // Sessão válida, libera o preloader
        window.__progeneseKeepPreloader = false;
        window.dispatchEvent(new CustomEvent('progenese:release-preloader'));
      }
    } catch (e) {
      window.location.replace(`${BASE_PREFIX}/progenese/login.html`);
    }
  ...
  async function updateWelcomeName() {
    ...
    try {
      ...
      if (data && data.ok && data.authenticated) {
        ...
        const display = nome || usuario || 'Usuário';
        span.textContent = display;
      }
    } catch (_) {
      // Mantém texto padrão se sessão não estiver acessível
    }
  }
