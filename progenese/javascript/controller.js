 ...
 */
  ...
  (async () => {
    if (currentEditingBoat) {
      // Edição: atualizar em memória e persistir progresso no backend quando possível
      ...
      try {
        if (!isTempId(currentEditingBoat.id)) {
          await apiRequest('editar_barco', { id: parseInt(currentEditingBoat.id, 10), numero_serie: codigo, cliente_nome: cliente, modelo, status_producao: progress }, 'POST');
        }
      } catch (e) {
        console.warn('[estaleiro] Falha ao persistir progresso do barco:', e);
      }
      hideModal();
      loadBoats();
      updateCounters();
