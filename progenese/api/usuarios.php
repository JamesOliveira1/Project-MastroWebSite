...
function startSecureSession(): void {
  ...
  // Garante que o Garbage Collector do PHP não apague a sessão antes de 30 dias
...
function enforceSessionTimeout(int $ttlSeconds = 1800): bool {
  ...
  $ttl = $remember ? (10 * 24 * 60 * 60) : $ttlSeconds; // 10 dias ou padrão
  $last = $_SESSION['last_activity'] ?? null;
  if ($last && (time() - (int)$last) > $ttl) {
    return false; // expired
  }

  // Se "manter logado" estiver ativo, renova o cookie para criar uma expiração deslizante (sliding expiration)
  if ($remember) {
...
try {
  ...
  switch ($action) {
    ...
    case 'login': {
      ...
      if ($remember) {
        // Atualiza cookie da sessão para 10 dias
        setcookie(session_name(), session_id(), [
          'expires' => time() + (10 * 24 * 60 * 60),
          'path' => '/',
          'domain' => '',
    ...
    case 'profile_update': {
      ...
      $novoNome = trim((string)($input['nome'] ?? $_POST['nome'] ?? ''));
      $novoUsuario = trim((string)($input['usuario'] ?? $_POST['usuario'] ?? ''));
      $novoEmail = trim((string)($input['email'] ?? $_POST['email'] ?? ''));
      $novaSenha = (string)($input['senha'] ?? $_POST['senha'] ?? '');

      $fields = [];
      $params = [':id' => $uid];

      if ($novoNome !== '') {
        if (mb_strlen($novoNome) > 120) jsonResponse(['ok' => false, 'error' => 'Nome muito longo'], 400);
        $fields[] = 'nome = :nome';
        $params[':nome'] = $novoNome;
      }

      if ($novoUsuario !== '') {
        if (mb_strlen($novoUsuario) < 3 || mb_strlen($novoUsuario) > 64) {
          jsonResponse(['ok' => false, 'error' => 'Usuário inválido'], 400);
        }
        $exists = queryOne('SELECT id FROM usuarios WHERE usuario = :u AND id <> :id LIMIT 1', [':u' => $novoUsuario, ':id' => $uid]);
        if ($exists) jsonResponse(['ok' => false, 'error' => 'Usuário já utilizado'], 409);
        $fields[] = 'usuario = :usuario';
        $params[':usuario'] = $novoUsuario;
      }
      ...
      if ($novoEmail !== '') {
        ...
        $exists = queryOne('SELECT id FROM usuarios WHERE email = :e AND id <> :id LIMIT 1', [':e' => $novoEmail, ':id' => $uid]);
        if ($exists) jsonResponse(['ok' => false, 'error' => 'E-mail já utilizado'], 409);
        $fields[] = 'email = :email';
      ...
      if ($novaSenha !== '') {
        if (strlen($novaSenha) < 8 || strlen($novaSenha) > 128) {
          jsonResponse(['ok' => false, 'error' => 'Senha inválida'], 400);
        }
      ...
      if (empty($fields)) {
        jsonResponse(['ok' => true, 'updated' => false]);
      }

      $sql = 'UPDATE usuarios SET ' . implode(', ', $fields) . ' WHERE id = :id';
      $affected = execute($sql, $params);
      if ($affected < 1) {
        jsonResponse(['ok' => false, 'error' => 'Nenhuma alteração aplicada'], 400);
      }

      // Atualiza sessão com novos valores
      if (isset($params[':nome'])) $_SESSION['nome'] = (string)$params[':nome'];
      if (isset($params[':usuario'])) $_SESSION['usuario'] = (string)$params[':usuario'];
      if (isset($params[':email'])) $_SESSION['email'] = (string)$params[':email'];
