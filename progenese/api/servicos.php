...
try {
  ...
  switch ($action) {
    ...
    case 'servicos_importar': {
      $barco_id = getInt($input + $_POST, 'barco_id', 0);
      $cats = $input['categorias'] ?? $_POST['categorias'] ?? [];
      if ($barco_id <= 0 || !is_array($cats)) jsonResponse(['ok' => false, 'error' => 'barco_id e categorias obrigatórios'], 400);
      $pdo = getConnection();
      try {
        $pdo->beginTransaction();
        execute('DELETE FROM servicos WHERE barco_id = :b', [':b' => $barco_id]);
        execute('DELETE FROM categoria_servicos WHERE barco_id = :b', [':b' => $barco_id]);
        $numero = 1;
        foreach ($cats as $c) {
          $nome = trim((string)($c['nome'] ?? 'Categoria'));
          execute('INSERT INTO categoria_servicos (barco_id, nome, numero) VALUES (:b, :n, :num)', [':b' => $barco_id, ':n' => $nome, ':num' => $numero]);
          $catId = (int)$pdo->lastInsertId();
          $servs = is_array($c['servicos'] ?? null) ? $c['servicos'] : [];
          $pos = 1;
          foreach ($servs as $s) {
            $desc = trim((string)($s['nome'] ?? 'Serviço'));
            execute('INSERT INTO servicos (barco_id, descricao, categoria_id, status, posicao, data_inicio, data_concluido, observacao) VALUES (:b, :d, :c, :s, :p, NULL, NULL, NULL)', [
              ':b' => $barco_id,
              ':d' => $desc,
              ':c' => $catId,
              ':s' => 'Pendente',
        ...
        jsonResponse(['ok' => false, 'error' => 'Falha ao importar: ' . $e->getMessage()], 500);
      }
    }

    case 'controles_listar': {
      $barco_id = getInt($_GET + $_POST + $input, 'barco_id', 0);
      if ($barco_id <= 0) jsonResponse(['ok' => false, 'error' => 'barco_id obrigatório'], 400);
      $rows = queryAll('SELECT s.id AS servico_id, s.descricao AS servico, c.resultado, c.link, c.responsavel, c.data_verificacao, c.comentario FROM servicos s INNER JOIN controles c ON c.servico_id = s.id WHERE s.barco_id = :b ORDER BY s.categoria_id, COALESCE(s.posicao, s.id)', [':b' => $barco_id]);
    ...
    case 'upload_controle': {
      ...
      $prev = queryOne('SELECT link FROM controles WHERE servico_id = :s', [':s' => $servico_id]);
      if ($prev && !empty($prev['link'])) {
        $oldBase = basename((string)$prev['link']);
        $oldPath = $dir . '/' . $oldBase;
