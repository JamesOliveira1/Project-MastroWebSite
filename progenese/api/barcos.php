...
try {
    ...
    switch ($action) {
        ...
        case 'criar_barco': {
            ...
            // Log de atividade: adicionar barco
            try {
                $desc = 'adicionou barco #' . $id . ' — ' . (string)($row['modelo'] ?? '') . ' — ' . (string)($row['cliente_nome'] ?? '');
                logActivity('adicionar', 'barcos', $id, $id, $desc);
            } catch (Throwable $_) {}
            jsonResponse(['ok' => true, 'data' => $row]);
        }
        case 'editar_barco': {
            ...
            // Permite atualização parcial: se modelo/cliente não enviados, carrega existentes
            if ($modelo === '' || $cliente_nome === '') {
                $prev = queryOne('SELECT numero_serie, cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $id]);
                if (!$prev) { jsonResponse(['ok' => false, 'error' => 'Barco não encontrado'], 404); }
                if ($modelo === '') { $modelo = (string)($prev['modelo'] ?? ''); }
                if ($cliente_nome === '') { $cliente_nome = (string)($prev['cliente_nome'] ?? ''); }
                if ($numero_serie === '') { $numero_serie = (string)($prev['numero_serie'] ?? ''); }
                if ($modelo === '' || $cliente_nome === '') {
                    jsonResponse(['ok' => false, 'error' => 'Campos obrigatórios: modelo e cliente_nome'], 400);
                }
            }
            ...
            $row = queryOne('SELECT id, numero_serie, cliente_nome, modelo, status_producao, criado_em FROM barcos WHERE id = :id', [':id' => $id]);
            // Log de atividade: editar barco
            try {
                $desc = 'editou barco #' . $id . ' — ' . (string)($row['modelo'] ?? '') . ' — ' . (string)($row['cliente_nome'] ?? '');
                logActivity('editar', 'barcos', $id, $id, $desc);
            } catch (Throwable $_) {}
            jsonResponse(['ok' => true, 'data' => $row]);
        }
        case 'excluir_barco': {
            ...
            // Busca dados para descrição antes de excluir
            $prev = queryOne('SELECT id, cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $id]);
            $affected = execute('DELETE FROM barcos WHERE id = :id', [':id' => $id]);
            if ($affected < 1) {
                jsonResponse(['ok' => false, 'error' => 'Barco não encontrado'], 404);
            }
            // Log de atividade: excluir barco
            try {
                $desc = 'deletou barco #' . $id . ' — ' . (string)($prev['modelo'] ?? '') . ' — ' . (string)($prev['cliente_nome'] ?? '');
                logActivity('excluir', 'barcos', $id, $id, $desc);
            } catch (Throwable $_) {}
        ...
        case 'salvar_informacoes': {
            ...
            $out = queryOne('SELECT 
                             ...
                             ORDER BY i.id DESC
                             LIMIT 1', [':id' => $barcoId]);
            // Log de atividade: editar ficha técnica
            try {
                $desc = 'editou ficha técnica do barco #' . $barcoId . ' — ' . (string)($out['modelo'] ?? '') . ' — ' . (string)($out['cliente_nome'] ?? '');
                logActivity('editar', 'informacoes', $barcoId, $barcoId, $desc);
            } catch (Throwable $_) {}
        ...
        case 'adicionar_informacao_extra': {
            ...
            // Log de atividade: adicionar informação extra
            try {
                $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $barcoId]);
                $desc = 'adicionou informação extra ("' . $nome . '") em barco #' . $barcoId . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
                logActivity('adicionar', 'informacoes_extras', $id, $barcoId, $desc);
            } catch (Throwable $_) {}
            jsonResponse(['ok' => true, 'data' => $row]);
        }
        case 'excluir_informacao_extra': {
            ...
            // Obter barco_id e nome para log
            $prev = queryOne('SELECT barco_id, nome FROM informacoes_extras WHERE id = :id', [':id' => $id]);
            ...
            // Log de atividade: excluir informação extra
            try {
                $boat = null;
                $barcoId = isset($prev['barco_id']) ? (int)$prev['barco_id'] : null;
                if ($barcoId) { $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $barcoId]); }
                $desc = 'excluiu informação extra ("' . (string)($prev['nome'] ?? '') . '") em barco #' . ($barcoId ?? 0) . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
                logActivity('excluir', 'informacoes_extras', $id, $barcoId, $desc);
            } catch (Throwable $_) {}
            jsonResponse(['ok' => true, 'deleted' => $id]);
        }
        default:
