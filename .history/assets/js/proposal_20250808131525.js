...
// Formata o valor da proposta com separador de milhar e vírgula para centavos
document.addEventListener('DOMContentLoaded', function () {
    const input = document.getElementById('proposalvalue');
    function formatValue(raw) {
    let value = raw.replace(/\D/g, '');
    if (!value) return '0,00';
    // Se o valor é menor que 1000, não adicione zeros à esquerda
    if (value.length < 4) {
        return value + ',00';
    }
    // Se o valor foi colado e não tem centavos, adiciona ,00
    let intPart = value;
    let decPart = '00';
    if (raw.includes(',') || raw.includes('.')) {
        // Se já tem separador decimal, trata normalmente
        intPart = value.slice(0, -2);
        decPart = value.slice(-2);
    }
