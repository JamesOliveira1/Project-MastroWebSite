...
function updateTotals() {
  ...
  const fmtVal = (val) => fmt(val).replace(/\./g, "").replace(",", ".").replace(".", ","); // Formato brasileiro 1.000,00

  // Atualiza APENAS os campos ocultos para envio
  // O formato esperado pelo backend/email parece ser o formatado (ex: "1.234,56")
  document.getElementById("hiddenAssemblyTotal").value = fmt(currentTotals.assembly);
  document.getElementById("hiddenMotorizationTotal").value = fmt(currentTotals.motor);
  document.getElementById("hiddenOptionsTotal").value = fmt(currentTotals.options);
  document.getElementById("hiddenTotalPrice").value = fmt(total);
