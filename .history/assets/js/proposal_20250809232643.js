...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (select && typeof mastroproposta === 'object') {
        ...
        select.addEventListener('change', function () {
            ...
            if (selected && mastroproposta[selected]) {
                ...
                if (seriesField) {
    const maxLineLength = 25; // Máximo de caracteres por linha

    function quebrarTextoPorTamanho(texto, limite) {
        const palavras = texto.split(" ");
        let linhas = [];
        let linhaAtual = "";

        palavras.forEach(palavra => {
            if ((linhaAtual + palavra).length + 1 <= limite) {
    ...
    let texto = (mastroproposta[selected].serieitens || [])
        .map(function(item) {
            return quebrarTextoPorTamanho(item.name, maxLineLength);
        })
        .join("\n");

    seriesField.value = texto;
}
