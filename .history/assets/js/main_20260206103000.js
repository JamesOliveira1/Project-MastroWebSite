...
window.addEventListener('load', initProgeneseNavEnhancements);
  ...
  if (preloader) {
    window.addEventListener('load', () => {
      // O preloader será removido/ocultado pelo custom.js quando os dados terminarem de carregar
      // Se não estivermos na página de customização (onde o custom.js controla), removemos normalmente
      if (!window.location.pathname.includes('custom.html') && !window.location.pathname.includes('/custom')) {
          preloader.remove()
      } else {
          // Na página custom, apenas garantimos que ele inicie visível, o custom.js cuidará de removê-lo
      }
    });
  }
