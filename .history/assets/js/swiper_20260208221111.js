    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            if (data && data[modelName]) {
                ...
                // Popula as seções
                populateModelInfo(boat);
                populateSeriesItems(boat);
                populatePerformance(boat);
                
                // Dispara evento para atualizar traduções se necessário (re-scan do i18next)
                // Se o i18n-setup.js expuser alguma função de reload ou se usarmos jqueryI18next:
                if (window.jqueryI18next && window.i18next) {
                     $('body').localize();
                }
            }
        })
...
function populateModelInfo(boat) {
    ...
    const fields = [
        { key: 'categoria', label: 'Categoria', i18n: 'portfolio_pages.labels.category', val: boat.categoria },
...
function populatePerformance(boat) {
    ...
    const perfData = [
        { 
            val: boat.velocidade_cruzeiro ? fmt(boat.velocidade_cruzeiro) + ' NÓS' : '', 
            label: 'Velocidade de Cruzeiro Econômico',
            i18n: 'portfolio_pages.labels.cruise_speed'
        },
        { 
            val: boat.autonomia ? fmt(boat.autonomia) + ' MILHAS' : '', 
            label: 'Autonomia em Cruzeiro Econômico',
            i18n: 'portfolio_pages.labels.cruise_range'
        },
        { 
            val: boat.velocidade_maxima ? fmt(boat.velocidade_maxima) + ' NÓS' : '', 
