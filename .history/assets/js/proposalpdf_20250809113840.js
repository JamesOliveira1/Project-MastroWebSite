...
async function gerarPDF() {
  ...
  // CORREÇÃO: Função addImageCover que estava faltando
  async function addImageCover(doc, imgSrc, x, y, targetWmm, targetHmm) {
    try {
      ...
      console.error(`Erro ao carregar imagem do motor: ${imgSrc}`, error);
      throw error; // Re-throw para que o erro seja capturado pelo try/catch principal
    }
  }
...
// CORREÇÃO: Lógica melhorada para capturar e carregar a imagem do motor
let motorImgPath = document.getElementById("proposalmotorimg")?.value;

// Se não há caminho definido, tentar construir baseado no motor selecionado
if (!motorImgPath || motorImgPath.trim() === "") {
  if (motorConfig && motorConfig !== "—" && motorConfig.trim() !== "") {
    // Limpar o nome do motor para usar como nome do arquivo
    const motorFileName = motorConfig
      .replace(/\s+/g, "_")           // Substituir espaços por underscore
      .replace(/[^\w\-_.]/g, "")     // Remover caracteres especiais
      .toLowerCase();                // Converter para minúsculas
    
    motorImgPath = `assets/img/motor/${motorFileName}.jpg`;
  }
}
...
if (motorImgPath && motorImgPath.trim() !== "") {
  try {
    console.log(`Tentando carregar imagem do motor: ${motorImgPath}`);
    await addImageCover(doc, motorImgPath, 232, 201, 45, 88);
    console.log("Imagem do motor adicionada com sucesso!");
  } catch (error) {
    console.warn("Erro ao carregar imagem do motor:", motorImgPath, error);
    
    // Tentar caminhos alternativos
    const alternativePaths = [
      `assets/img/motor/${motorConfig}.png`,
      `assets/img/motor/${motorConfig.toLowerCase()}.jpg`,
      `assets/img/motor/${motorConfig.toLowerCase()}.png`,
      `assets/img/motor/default.jpg`,
      `assets/img/motor/motor_default.jpg`
    ];
    
    for (const altPath of alternativePaths) {
      try {
        console.log(`Tentando caminho alternativo: ${altPath}`);
        await addImageCover(doc, altPath, 232, 201, 45, 88);
        console.log(`Sucesso com caminho alternativo: ${altPath}`);
        break;
      } catch (altError) {
        console.warn(`Falha no caminho alternativo: ${altPath}`);
      }
    }
  }
} else {
