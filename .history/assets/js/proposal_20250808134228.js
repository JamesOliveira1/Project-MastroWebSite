...
document.addEventListener('DOMContentLoaded', function () {
  ...
  input.addEventListener('blur', ensureTwoDecimals);
  ...
  function formatInput(e) {
    ...
    // Se já tem vírgula, divide parte inteira e decimal
    let [intPart, decPart] = raw.split(',');

    // Garante no máximo dois dígitos na parte decimal
    ...
    e.target.value = decPart ? `${intPart},${decPart}` : `${intPart},00`;
  }

  function ensureTwoDecimals(e) {
    ...
    if (!val.includes(',')) {
      ...
      decPart = (decPart + '00').substring(0, 2); // completa 2 casas
