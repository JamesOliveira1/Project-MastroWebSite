...
if (foto3) {
 ...
 setBoldStyle();
doc.text(safeSplitTextToSize(modelInfo, 260), 22, 95, { baseline: "top" });
doc.text(safeSplitTextToSize(seriesItems, 260), 122, 95, { baseline: "top" });
doc.text(safeSplitTextToSize(optionsText, 260), 207, 95, { baseline: "top" });
...
if (motorConfig && motorConfig !== "—") {
  // Monta o caminho baseado no nome do motor
  const motorImagePath = `assets/img/motor/${motorConfig}.jpg`;

  try {
    await addImageCover(doc, motorImagePath, 232, 201, 45, 88); // W=45mm, H=88mm
  } catch (e) {
    console.warn("Imagem do motor não encontrada:", motorImagePath);
  }
}
