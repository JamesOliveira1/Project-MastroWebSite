...
// Função para inserir imagem carregada totalmente
async function addFixedImage(doc, imgSrc, x, y, wmm, hmm) {
  try {
    const img = new Image();
    img.src = imgSrc;
    await img.decode(); // garante que a imagem está carregada

    doc.addImage(img, "JPEG", x, y, wmm, hmm);
  } catch (error) {
    console.warn("Erro ao carregar imagem:", imgSrc, error);
  }
}

// Adiciona fotos com tamanhos fixos
if (foto1) await addFixedImage(doc, foto1, 18, 90, 133, 95);
if (foto2) await addFixedImage(doc, foto2, 158, 90, 80, 45);
if (foto3) await addFixedImage(doc, foto3, 158, 140, 80, 45);
