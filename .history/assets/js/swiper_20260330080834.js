...
function populateSeriesItems(boat) {
    ...
    const locale = (window.i18next && i18next.language) ? i18next.language : undefined;
    let items = boat.itens ? boat.itens.split(',').map(s => s.trim()).filter(s => s) : [];
    items = items.sort((a, b) => {
        const ta = getDbTranslation(a);
        const tb = getDbTranslation(b);
