...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Detecta o caminho base para os arquivos de tradução baseado na localização deste script
  let loadPath = 'assets/locales/{{lng}}.json'; // Fallback padrão
  
  // Procura pelo script i18n-setup.js para determinar o caminho absoluto dos assets
  const scripts = document.getElementsByTagName('script');
  for (let i = 0; i < scripts.length; i++) {
    if (scripts[i].src && scripts[i].src.includes('i18n-setup.js')) {
       // Assume que a estrutura é .../assets/js/i18n-setup.js
       // Queremos transformar em .../assets/locales/{{lng}}.json
       const scriptUrl = scripts[i].src;
       console.log('i18n: Script URL encontrada:', scriptUrl);
       // Substitui 'js/i18n-setup.js' (e possíveis query params) por 'locales/{{lng}}.json'
       if (scriptUrl.match(/js\/i18n-setup\.js(\?.*)?$/)) {
           loadPath = scriptUrl.replace(/js\/i18n-setup\.js(\?.*)?$/, 'locales/{{lng}}.json');
           console.log('i18n: LoadPath detectado:', loadPath);
           break;
       }
    }
  }
