        ...
        .then(data => {
            ...
            Object.keys(data).forEach(modelName => {
                if (mastroproposta[modelName]) {
                    ...
                    // Helper para formatar valores (remove .0 ou .00 se for inteiro)
                    const fmt = (val) => {
                        if (!val) return '';
                    ...
                    if (boat.peso) info.push({ name: `Peso: ${fmt(boat.peso)}Kg` });
                    if (boat.agua) info.push({ name: `Água: ${fmt(boat.agua)}L` });
                    if (boat.combustivel) info.push({ name: `Combustível: ${fmt(boat.combustivel)}L` });
