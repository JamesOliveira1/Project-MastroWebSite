...
doc.setFontSize(47);
doc.setTextColor(white[0], white[1], white[2]);
doc.text(model, 166, 67);

// Texto ano
setBoldStyle();
doc.setFont("helvetica", "bold");
doc.setFontSize(32);
doc.text(year, 243, 80);
...
// Foto 1: 90mm x 19mm em (18, 185)
if (foto1) {
  await addImageCover(doc, foto1, 18, 185, 90, 19);
}

// Foto 2: 156mm x 90mm em (156, 133)
if (foto2) {
  await addImageCover(doc, foto2, 156, 133, 156, 90);
}

// Foto 3: 156mm x 90mm em (156, 185)
if (foto3) {
  await addImageCover(doc, foto3, 156, 185, 156, 90);
}
  ...
  doc.text(safeSplitTextToSize(motorConfig, 260), 173, 155);
  ...
  // Cliente 
  setBoldStyle();
  doc.text(client, 138, 28);

  // Prazo de entrega 
  setBoldStyle();
  const deliveryLines = safeSplitTextToSize(delivery, 120);
  doc.text(deliveryLines, 94, 98);

  // Data 
  setBoldStyle();
  doc.text(formatDate(date), 19, 157);

  // Validade
  setBoldStyle();
  doc.text(validity, 68, 162);
