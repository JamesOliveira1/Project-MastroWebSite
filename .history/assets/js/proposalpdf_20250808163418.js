...
async function gerarPDF() {
  ...
  doc.setFontSize(32);
  doc.setTextColor(white[0], white[1], white[2]);
  doc.text(`${currency} ${value}`, 143, 52);

  // Descrição do valor 
  setWhiteBoldStyle();
  doc.text(valuedesc, 200, 70);

  // Condições de pagamento
  setWhiteBoldStyle();
  const paymentLines = safeSplitTextToSize(payment, 120);
  doc.text(paymentLines, 134, 124);

  // Observações
  setWhiteBoldStyle();
  const notesLines = safeSplitTextToSize(notes, 120);
  doc.text(notesLines, 134, 180);
