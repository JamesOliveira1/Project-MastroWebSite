...
// Função para adicionar imagem mantendo proporção, sem ultrapassar o tamanho máximo
async function addImageFit(doc, imgSrc, x, y, maxWmm, maxHmm) {
  const img = new Image();
  img.src = imgSrc;
  await img.decode();
  ...
  // Calcula escala máxima que caiba no retângulo
  const scaleW = maxWmm / (img.width / mmToPt);
  const scaleH = maxHmm / (img.height / mmToPt);
  const scale = Math.min(scaleW, scaleH);

  // Calcula tamanho final
  const drawW = (img.width / mmToPt) * scale;
  const drawH = (img.height / mmToPt) * scale;

  // Adiciona no PDF
  doc.addImage(img, "JPEG", x, y, drawW, drawH);
}

// Página 3 com tamanhos máximos
if (foto1) {
  await addImageFit(doc, foto1, 18, 90, 133, 95);
}
if (foto2) {
  await addImageFit(doc, foto2, 158, 90, 80, 45);
}
if (foto3) {
  await addImageFit(doc, foto3, 158, 140, 80, 45);
}
...
setWhiteBoldStyle()
doc.setFontSize(13);
doc.text(safeSplitTextToSize(modelInfo, 260), 22, 95.18, { baseline: "top", lineHeightFactor: 2.0 });
setBoldStyle();
doc.setFontSize(13);
doc.text(safeSplitTextToSize(seriesItems, 260), 120, 95.18, { baseline: "top", lineHeightFactor: 2.0 });
doc.text(safeSplitTextToSize(optionsText, 260), 213, 95.16, { baseline: "top", lineHeightFactor: 2.0 }); 
...
const startX = 115;      // posição inicial X (mm)
const startY = 42;      // posição inicial Y (mm)
const maxWidth = 153;   // largura máxima do texto (mm)
...
// Configuração básica (lado esquerdo)
setBoldStyle();
doc.text(safeSplitTextToSize(basicSetup, 260), 20, 40);

// Motor Config (lado direito) - Enquadrado com largura máxima de 142mm
setBoldStyle();
// Quebrar o texto respeitando a largura máxima de 142mm
const motorConfigLines = safeSplitTextToSize(motorConfig, 142);

// Posicionar o texto alinhado à direita, com ponto superior esquerdo em (275, 36)
// Para alinhamento à direita, calculamos a posição X baseada na largura máxima
const rightAlignX = 275; // Posição X do canto direito da área
doc.text(motorConfigLines, rightAlignX, 36, { 
  align: "right",
  baseline: "top"
});
...
if (motorImgPath && motorImgPath.trim() !== "") {
  try {
    ...
    const img = new Image();
    img.src = motorImgPath;
    await img.decode();

    // Conversão de mm para pontos (usado no jsPDF)
    const mmToPt = 2.83465;

    // Largura fixa em mm
    const targetWmm = 35;
    // Calcula altura proporcional para manter aspecto original
    const aspectRatio = img.width / img.height;
    const targetHmm = targetWmm / aspectRatio;

    // Adiciona imagem na posição desejada (top-left = 232, 114)
    doc.addImage(
      img,
      "JPEG",
      232,             // X inicial (mm)
      119,             // Y inicial (mm)
      targetWmm,       // Largura em mm
      targetHmm        // Altura proporcional em mm
    );
  ...
  // Nome do Cliente 
  setBoldStyle();
  doc.text(client, 138, 27.47);

  // Prazo de entrega 
  setBoldStyle();
  const deliveryLines = safeSplitTextToSize(delivery, 120);
  doc.text(deliveryLines, 94, 96.49);
