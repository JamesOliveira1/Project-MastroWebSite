...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Verifica o reCAPTCHA com a API do Google (cURL com fallback para stream)
    ...
    if ($recaptchaSecret && $recaptchaToken) {
        ...
        // Loga ausência de secret/token
        (function () use ($recaptchaSecret, $recaptchaToken) {
            $dir = __DIR__ . '/../logs';
            if (!is_dir($dir)) { @mkdir($dir, 0755, true); }
            $data = [
                'status' => 'recaptcha_misconfig',
                'has_secret' => (bool) $recaptchaSecret,
                'has_token' => (bool) $recaptchaToken,
    ...
    if (!empty($recaptchaData['success']) && ($recaptchaData['score'] ?? 0) >= $recaptchaMinScore) {
        ...
        // Loga detalhes do recaptcha para diagnóstico sem expor token
        (function ($recaptchaData) {
            ...
            @file_put_contents($dir . '/send_orcamento.log', '[' . date('Y-m-d H:i:s') . '] ' . json_encode($safe, JSON_UNESCAPED_UNICODE) . PHP_EOL, FILE_APPEND);
        })($recaptchaData);
    }
} else {
