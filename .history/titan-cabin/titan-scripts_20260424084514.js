...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (titanForm) {
    titanForm.addEventListener('submit', function(e) {
      e.preventDefault();
      
      const submitBtn = titanForm.querySelector('button[type="submit"]');
      ...
      // Collect Form Data
      const formData = new FormData(titanForm);
      ...
      if (selectedEngine) {
        ...
        formData.append('engine', 'Nenhum motor selecionado');
      }

      // Collect selected options
      let selectedOptions = [];
      document.querySelectorAll('.options-list input[type="checkbox"]:checked').forEach(cb => {
        let text = cb.parentNode.textContent.trim();
      ...
      if (selectedOptions.length > 0) {
        ...
        formData.append('options', 'Nenhum opcional selecionado');
      }

      // Add a hardcoded subject
      formData.append('subject', 'Orçamento Titan Cabin');

      // Send Request
      fetch(titanForm.action, {
        method: 'POST',
        body: formData
      })
      ...
      .then(data => {
        ...
        if (data.success) {
          formMessage.innerHTML = '<div class="alert alert-success">Formulário enviado com sucesso! Entraremos em contato em breve.</div>';
          titanForm.reset();
          // Uncheck engine and options
          if (selectedEngine) selectedEngine.checked = false;
          document.querySelectorAll('.options-list input[type="checkbox"]').forEach(cb => cb.checked = false);
        } else {
          formMessage.innerHTML = '<div class="alert alert-danger">' + (data.error || 'Erro ao enviar. Tente novamente.') + '</div>';
        }
      })
      .catch(error => {
        console.error('Error:', error);
        formMessage.style.display = 'block';
        formMessage.innerHTML = '<div class="alert alert-danger">Erro de conexão. Tente novamente mais tarde.</div>';
      })
      .finally(() => {
