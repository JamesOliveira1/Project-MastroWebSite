  ...
  try {
    ...
    (occ || []).forEach(o => {
      ...
      if (key === 'laminacao' || key === 'montagem') {
        ...
        trashBoats.push({ id: b.id, codigo: b.codigo, modelo: b.modelo, cliente: b.cliente, progress: b.progress || 0 });
      }
    });
  } catch (e) {
    ...
    function setStatusMessage(text, type = 'info') {
      ...
      if (!el) { console.warn('[estaleiro] statusBanner not found'); return; }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      console.log('[estaleiro] DOMContentLoaded fired');
      const isEstaleiroPage = !!document.querySelector('.boat-slot') || !!document.getElementById('boatModal');
      console.log('[estaleiro] isEstaleiroPage =', isEstaleiroPage);
      if (isEstaleiroPage) {
        initializeEstaleiroSystem();
        // Show initial sync message on page enter
        setStatusMessage('sistema sincronizado com o db', 'info');
        fetchBoatsFromAPI()
...
+      console.log('[estaleiro] initializeEstaleiroSystem start');
      // Modal save button
      const saveBoatBtnEl = document.getElementById('saveBoatBtn');
      if (saveBoatBtnEl) saveBoatBtnEl.addEventListener('click', saveBoat);
      // Modal delete button
      const deleteBoatBtnEl = document.getElementById('deleteBoatBtn');
      if (deleteBoatBtnEl) deleteBoatBtnEl.addEventListener('click', deleteBoat);
      
      // Progress slider
      const boatProgressEl = document.getElementById('boatProgress');
      if (boatProgressEl) {
        boatProgressEl.addEventListener('input', function() {
          const progressValueEl = document.getElementById('progressValue');
          if (progressValueEl) progressValueEl.textContent = this.value + '%';
      ...
      if (saveChangesBtn) {
        console.log('[estaleiro] saveChangesBtn found, binding click');
        saveChangesBtn.addEventListener('click', function(ev){
          console.log('[estaleiro] saveChangesBtn clicked');
          setStatusMessage('salvando...', 'info');
          try { saveOccupancy(); } catch(e){ console.error('[estaleiro] saveOccupancy sync call error:', e); }
        });
      } else {
...
function buildOccupancyPayload() {
  const payload = [];
  boats.forEach(b => {
    if (b.warehouse && b.slot != null) {
      payload.push({ barco_id: parseInt(b.id, 10), local_nome: localKeyToNome(b.warehouse), numero: parseInt(b.slot, 10) });
    }
  });
  completedBoats.forEach(b => { payload.push({ barco_id: parseInt(b.id, 10), local_nome: 'Concluídos' }); });
  preBoats.forEach(b => { payload.push({ barco_id: parseInt(b.id, 10), local_nome: 'Pré-Projetos' }); });
  (trashBoats || []).forEach(b => { payload.push({ barco_id: parseInt(b.id, 10), local_nome: 'Lixeira' }); });
  return payload;
}
async function saveOccupancy() {
  try {
    console.log('[estaleiro] saveOccupancy start');
    setStatusMessage('salvando...', 'info');
    // Criar barcos novos primeiro
    const newCandidates = [];
    boats.forEach(b => { if (b.isNew || isTempId(b.id)) newCandidates.push(b); });
    preBoats.forEach(b => { if (b.isNew || isTempId(b.id)) newCandidates.push(b); });
    (completedBoats || []).forEach(b => { if (b.isNew || isTempId(b.id)) newCandidates.push(b); });
    (trashBoats || []).forEach(b => { if (b.isNew || isTempId(b.id)) newCandidates.push(b); });
    console.log('[estaleiro] newCandidates:', newCandidates.length);
    for (const b of newCandidates) {
      try {
        console.log('[estaleiro] creating boat tempId=', b.id);
        const resp = await apiRequest('criar_barco', { numero_serie: b.codigo || '', cliente_nome: b.cliente || '', modelo: b.modelo || '', status_producao: b.progress || 0 }, 'POST');
        const row = Array.isArray(resp) ? resp[0] : (resp && resp.data ? resp.data : null);
        if (!row || !row.id) throw new Error('Resposta inválida ao criar barco');
        ...
        console.log('[estaleiro] created boat id=', newId);
      } catch (e) {
        console.error('Falha ao criar barco novo:', e);
        setStatusMessage('erro ao salvar', 'danger');
        return;
      }
    }

    const payload = buildOccupancyPayload();
    console.log('[estaleiro] salvar_ocupacao payload length=', payload.length);
    await estaleiroRequest('salvar_ocupacao', { ocupacoes: payload }, 'POST');
    console.log('[estaleiro] salvar_ocupacao OK');
    setStatusMessage('salvo com sucesso', 'success');
