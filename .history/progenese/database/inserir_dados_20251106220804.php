...
try {
    ...
    if ($usuario == 0) {
        $stmt = $db->prepare("INSERT INTO usuarios (nome, usuario, senha, cargo) VALUES (?, ?, ?, ?)");
        $stmt->execute(['Administrador', 'admin', password_hash('1234', PASSWORD_DEFAULT), 'Gerente']);
    ...
    $usuariosExtras = [
        ['Leo', 'leo', '157.470@leo', 'Usuário'],
        ['Marcelo', 'marcelo', 'senha123', 'Usuário'],
        ['James', 'james', '12345678', 'Usuário'],
    ];

    $stmtExisteUsuario = $db->prepare("SELECT COUNT(*) FROM usuarios WHERE usuario = ?");
    $stmtInsereUsuario = $db->prepare("INSERT INTO usuarios (nome, usuario, senha, cargo) VALUES (?, ?, ?, ?)");

    foreach ($usuariosExtras as [$nome, $usuarioLogin, $senhaPura, $cargo]) {
        $stmtExisteUsuario->execute([$usuarioLogin]);
        if ($stmtExisteUsuario->fetchColumn() == 0) {
            $hash = password_hash($senhaPura, PASSWORD_DEFAULT);
            $stmtInsereUsuario->execute([$nome, $usuarioLogin, $hash, $cargo]);
