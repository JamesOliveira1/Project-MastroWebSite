...
CREATE TABLE documento_inventario (
  ...
  criado_em TEXT,
  FOREIGN KEY (barco_id) REFERENCES barcos(id),
  FOREIGN KEY (inventario_barco_id) REFERENCES inventario_barco(id) ON DELETE SET NULL
);
...
CREATE TABLE site_barco (
  ...
  passageiros INTEGER,
  pernoite INTEGER,
  velocidade_cruzeiro_nos REAL,
  autonomia_milhas REAL,
  velocidade_maxima_nos REAL,
  barco_montagem_valor REAL,  
  descricao TEXT,
  img TEXT
);

CREATE TABLE site_itens_de_serie (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  item TEXT NOT NULL
);

CREATE TABLE site_opcionais (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE rel_barco_itens_serie (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  barco_id INTEGER NOT NULL,
  item_serie_id INTEGER NOT NULL,
  FOREIGN KEY (barco_id) REFERENCES site_barco(id) ON DELETE CASCADE,
  FOREIGN KEY (item_serie_id) REFERENCES site_itens_de_serie(id) ON DELETE CASCADE
);

CREATE TABLE rel_barco_opcionais (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  barco_id INTEGER NOT NULL,
  opcional_id INTEGER NOT NULL,
  FOREIGN KEY (barco_id) REFERENCES site_barco(id) ON DELETE CASCADE,
  FOREIGN KEY (opcional_id) REFERENCES site_opcionais(id) ON DELETE CASCADE
);

CREATE TABLE rel_barco_motores (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  barco_id INTEGER NOT NULL,
  motor_id INTEGER NOT NULL,
  FOREIGN KEY (barco_id) REFERENCES site_barco(id) ON DELETE CASCADE,
  FOREIGN KEY (motor_id) REFERENCES site_motor(id) ON DELETE CASCADE
);
-- =======================================================
...
CREATE INDEX IF NOT EXISTS idx_documento_inventario_barco ON documento_inventario(barco_id);
CREATE INDEX IF NOT EXISTS idx_documento_inventario_item ON documento_inventario(inventario_barco_id);
...
-- Índices para update dos modelos no site e valores
CREATE UNIQUE INDEX IF NOT EXISTS idx_site_barco_modelo ON site_barco(modelo);
CREATE INDEX IF NOT EXISTS idx_site_barco_categoria ON site_barco(categoria);
CREATE INDEX IF NOT EXISTS idx_site_barco_passageiros ON site_barco(passageiros);

CREATE UNIQUE INDEX IF NOT EXISTS idx_site_itens_de_serie_item ON site_itens_de_serie(item);
CREATE UNIQUE INDEX IF NOT EXISTS idx_site_opcionais_opcional ON site_opcionais(opcional);
CREATE UNIQUE INDEX IF NOT EXISTS idx_site_motor_motor ON site_motor(motor);

CREATE INDEX IF NOT EXISTS idx_rel_barco_itens_serie_barco ON rel_barco_itens_serie(barco_id);
CREATE INDEX IF NOT EXISTS idx_rel_barco_itens_serie_item ON rel_barco_itens_serie(item_serie_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_rel_barco_itens_serie_unq ON rel_barco_itens_serie(barco_id, item_serie_id);

CREATE INDEX IF NOT EXISTS idx_rel_barco_opcionais_barco ON rel_barco_opcionais(barco_id);
CREATE INDEX IF NOT EXISTS idx_rel_barco_opcionais_opcional ON rel_barco_opcionais(opcional_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_rel_barco_opcionais_unq ON rel_barco_opcionais(barco_id, opcional_id);

CREATE INDEX IF NOT EXISTS idx_rel_barco_motores_barco ON rel_barco_motores(barco_id);
CREATE INDEX IF NOT EXISTS idx_rel_barco_motores_motor ON rel_barco_motores(motor_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_rel_barco_motores_unq ON rel_barco_motores(barco_id, motor_id);
