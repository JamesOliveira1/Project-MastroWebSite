...
+    const nextPre = [];
+    const nextTrash = [];
+    // Limpa posições de galpão antes de aplicar novas
+    boats.forEach(b => { delete b.warehouse; delete b.slot; });
+    (occ || []).forEach(o => {
+      const bid = String(o.barco_id);
+      const b = boats.find(x => x.id === bid);
...
+      } else if (key === 'preprojetos') {
+        nextPre.push({ id: b.id, codigo: b.codigo, modelo: b.modelo, cliente: b.cliente, progress: b.progress || 0 });
+      } else if (key === 'lixeira') {
+        nextTrash.push({ id: b.id, codigo: b.codigo, modelo: b.modelo, cliente: b.cliente, progress: b.progress || 0 });
+      }
+    });
+    // Só atualiza as listas quando terminar o processamento, evitando sumiço temporário
+    completedBoats = nextCompleted;
+    preBoats = nextPre;
+    trashBoats = nextTrash;
+    console.log('[estaleiro] Ocupação atualizada', { galpoes: boats.filter(b => b.warehouse).length, concluidos: completedBoats.length, pre: preBoats.length, lixo: (trashBoats || []).length });
   } catch (e) {
...
async function saveOccupancy() {
  ...
  try {
    // Validar: novos barcos devem estar em uma vaga (galpão + número)
    const allNew = [];
    [boats, preBoats, completedBoats, trashBoats].forEach(arr => {
      (arr || []).forEach(b => { if (b && (b.isNew || isTempId(b.id))) allNew.push(b); });
    });
    const invalidNew = allNew.filter(b => !(b.warehouse && b.slot != null));
    if (invalidNew.length > 0) {
      console.warn('[estaleiro] Novos barcos sem vaga:', invalidNew.map(b => b.codigo || b.modelo || b.id));
      setStatusMessage('Há novos barcos sem vaga. Arraste para um galpão antes de salvar.', 'warning');
      return;
    }
    // Criar somente os novos válidos (com vaga definida)
    for (const b of allNew) {
      try {
        ...
        +        setStatusMessage('erro ao salvar: ' + (e && e.message ? e.message : 'Falha ao criar barco'), 'danger');
        return;
      }
    }

    const payload = buildOccupancyPayload();
    console.log('[estaleiro] Occupancy payload', payload);
    ...
    setStatusMessage('erro ao salvar', 'danger');
    setStatusMessage('erro ao salvar: ' + (e && e.message ? e.message : 'Erro desconhecido'), 'danger');
