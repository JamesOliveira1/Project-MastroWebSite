...
function initEquipamentos() {
    ...
    const btnClear = document.getElementById('btnBuscaEquipClear');
    if (btnClear) {
        btnClear.style.display = 'none'; // Inicialmente oculto
        btnClear.addEventListener('click', limparBuscaEquipamento);
    }
}

function limparBuscaEquipamento() {
    // Resetar campos bloqueados e limpar valores
    const nomeInput = document.getElementById('equipNome');
    const codigoInput = document.getElementById('equipCodigo');
    if(nomeInput) {
        nomeInput.value = '';
    ...
    // Resetar botões de busca para o estado original
    const btnNome = document.getElementById('btnBuscaEquipNome');
    const btnCodigo = document.getElementById('btnBuscaEquipCodigo');
    if(btnNome) {
        btnNome.style.display = '';
        btnNome.innerHTML = '<i class="bi bi-search"></i>';
    }
    if(btnCodigo) {
        btnCodigo.style.display = '';
        btnCodigo.innerHTML = '<i class="bi bi-search"></i>';
    }

    // Esconder resultados da tabela
    const resultsContainer = document.getElementById('buscaEquipResultsContainer');
    const resultsBody = document.getElementById('buscaEquipResultsBody');
...
function limparBuscaEquipamento() {
    ...
    if(btnNome) {
        btnNome.innerHTML = '<i class="bi bi-search"></i>';
        btnNome.style.display = '';
        // Reatribuir onclick original se necessário, mas eles chamam performSearch que usa o input
    }
    if(btnCodigo) {
        btnCodigo.innerHTML = '<i class="bi bi-search"></i>';
        btnCodigo.style.display = '';
    ...
    // Limpar resultados da tabela mas manter container visível se desejar, ou esconder
    // O usuário disse "limpando a pesquisa e os inputs, e liberando eles para digitação"
    // Vamos esconder os resultados para "resetar a pesquisa"
    const resultsContainer = document.getElementById('buscaEquipResultsContainer');
    const resultsBody = document.getElementById('buscaEquipResultsBody');
    if(resultsContainer) resultsContainer.style.display = 'none';
    if(resultsBody) resultsBody.innerHTML = '';
...
function setupBuscaEquipamentos() {
    ...
    const performSearch = async (term, offset = 0) => {
        try {
            ...
            // Mostrar botão limpar quando a busca é iniciada
            const btnClear = document.getElementById('btnBuscaEquipClear');
