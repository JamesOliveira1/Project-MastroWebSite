  ...
  function renderCategorias() {
    ...
    categorias.forEach((cat, ci) => {
      ...
      cat.servicos.forEach((svc, si) => {
        ...
        cqSel.addEventListener('change', async e => { svc.cq = e.target.value; await fetch(apiBase, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'controle_editar', servico_id: svc.id, resultado: svc.cq }) }); });
        ...
        anexoReplace.addEventListener('click', () => { fileInput.value = ''; fileInput.click(); });
        function updateAnexoView(){
          ...
          anexoPlus.style.display = has ? 'none' : 'inline-flex';
          anexoReplace.style.display = has ? 'inline-flex' : 'none';
          if (!has && anexoClip){
