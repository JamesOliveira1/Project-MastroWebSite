...
function renderModels(models) {
    ...
    if (models.length) {
        ...
        displayModelDetails(models[0].id);
    } else {
...
function renderRelateCheckboxes() {
    ...
    allSeriesItems.forEach((item, idx) => {
        ...
        cb.checked = relSeries.includes(id);
        cb.onchange = function () {
            if (this.checked) {
                if (!relSeries.includes(id)) relSeries.push(id);
            } else {
                relSeries = relSeries.filter(x => x !== id);
            }
        };
    ...
    allOptionals.forEach((item, idx) => {
        ...
        cb.type = 'checkbox';
        cb.className = 'form-check-input';
        cb.checked = relOptionals.includes(id);
        cb.onchange = function () {
            if (this.checked) {
                if (!relOptionals.includes(id)) relOptionals.push(id);
            } else {
                relOptionals = relOptionals.filter(x => x !== id);
            }
        };
        const span = document.createElement('span');
        span.className = 'form-check-label';
    ...
    allMotors.forEach((item, idx) => {
        ...
        cb.type = 'checkbox';
        cb.className = 'form-check-input';
        cb.checked = relMotors.includes(id);
        cb.onchange = function () {
            if (this.checked) {
                if (!relMotors.includes(id)) relMotors.push(id);
            } else {
                relMotors = relMotors.filter(x => x !== id);
            }
        };
        const span = document.createElement('span');
        span.className = 'form-check-label';
...
function saveRelations() {
    ...
    .then(d => {
        if (d.success) {
            ...
            console.error('Erro ao salvar relações:', d.message);
        }
    })
    .catch(e => console.error('Erro ao salvar relações:', e))
    .finally(() => togglePreloader(false));
}
