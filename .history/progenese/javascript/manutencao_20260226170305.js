...
function setupBuscaEquipamentos() {
    ...
    const performSearch = async (term) => {
        try {
            ...
            const response = await fetch(`api/manutencao.php?action=search_inventory&term=${encodeURIComponent(term)}`);
            if (!response.ok) throw new Error('Erro na busca');
            
            const data = await response.json();
            
            // Mapear retorno da API
            currentSearchResults = data.map(i => ({
                codigo: i.codigo || '-',
                item: i.item || 'Sem Nome'
            }));
    ...
    const renderSearchResults = () => {
        ...
        resultsBody.innerHTML = pageItems.map(item => `
            <tr>
                ...
                <td>${item.item}</td>
                <td class="text-end">
                    <button type="button" class="btn btn-sm btn-link p-0 text-primary" onclick="selecionarEquipBusca('${item.codigo}', '${item.item.replace(/'/g, "\\'")}')">
                        Selecionar
                    </button>
                </td>
            </tr>
...
window.selecionarEquipBusca = (codigo, item) => {
    ...
    // Desabilitar inputs para impedir edição após seleção
...
function editarEquipamento(id) {
    ...
    codigoInput.value = equip.codigo || '';
    
    // Desabilitar edição de Nome e Código no modo Edição
    nomeInput.readOnly = true;
    codigoInput.readOnly = true;
    nomeInput.classList.add('bg-light');
    codigoInput.classList.add('bg-light');
    // Desabilitar botões de busca também
    const btnBuscaNome = document.getElementById('btnBuscaEquipNome');
    const btnBuscaCodigo = document.getElementById('btnBuscaEquipCodigo');
    if(btnBuscaNome) btnBuscaNome.disabled = true;
    if(btnBuscaCodigo) btnBuscaCodigo.disabled = true;
