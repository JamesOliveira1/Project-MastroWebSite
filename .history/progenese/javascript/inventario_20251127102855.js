...
function renderSearchResults(items, meta) {
  ...
  tbody.innerHTML = '';
  if (!items || items.length === 0) {
    if (status) status.textContent = 'Nenhum item encontrado.';
    return;
  }
  if (status) {
    const off = meta && Number.isFinite(meta.offset) ? meta.offset : 0;
    const count = items.length;
    status.textContent = `Mostrando ${off + 1}–${off + count}`;
  }
  items.forEach(function(it) {
    ...
    tr.innerHTML = `
      ...
      <td>${it.nome || ''}</td>
      <td>${it.unidade_medida || '—'}</td>
      <td><button type="button" class="btn btn-sm btn-outline-primary">Selecionar</button></td>
    `;
    const btn = tr.querySelector('button');
    btn.addEventListener('click', function() {
      SELECTED_INV_ITEM = {
      ...
      if (status) status.textContent = `Selecionado: ${it.codigo_produto} — ${it.nome}`;
    });
    tbody.appendChild(tr);
  });
}
...
function setupAddInventarioModal() {
  ...
  if (!pagerDiv) {
    ...
    pagerDiv.className = 'd-flex justify-content-center gap-2 mt-2';
    pagerDiv.innerHTML = `
  ...
  const updatePager = () => {
    ...
    // Desativa avanço quando não há resultados; mantém ativo se houver qualquer item
    if (nextBtn) nextBtn.disabled = lastCount === 0;
  };
