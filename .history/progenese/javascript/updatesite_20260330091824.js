...
function renderSeries(items) {
    ...
    items.forEach(it => {
        ...
        delIcon.className = 'bi bi-x-lg';
        delIcon.style.fontSize = '1.1rem';
        btnDel.appendChild(delIcon);
        if (it.related && Number(it.related) === 1) {
            btnDel.setAttribute('aria-disabled', 'true');
            btnDel.title = 'Não é possível excluir: item relacionado';
            delIcon.title = 'Não é possível excluir: item relacionado';
            btnDel.style.cursor = 'default';
            btnDel.classList.add('text-muted');
        } else {
