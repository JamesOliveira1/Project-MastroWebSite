...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Exportar documentos do inventário (Excel)
  try {
    const btnExportDoc = document.getElementById('btnExportDocInventario');
    if (btnExportDoc) {
      btnExportDoc.addEventListener('click', function() {
        const barcoId = getBarcoIdFromURL();
        if (!barcoId || barcoId <= 0) {
          alert('Barco inválido para exportação.');
          return;
        }
        const url = `./api/inventario.php?action=exportar_excel_documento_inventario&barco_id=${encodeURIComponent(barcoId)}`;
        window.location.href = url;
      });
    }
  } catch (e) {
    console.warn('Falha ao configurar exportação de documentos do inventário:', e);
  }
        ...
        if (selectEl) {
          ...
          if (barcoId && barcoId > 0) {
            try {
              ...
              rows.forEach(r => {
                const opt = document.createElement('option');
                opt.value = String(r.id || r.inventario_id || '');
...
function appendDocumentoInventarioRow(r) {
  ...
  tr.innerHTML = `
    ...
    <td style="padding-right:20px;">${r.link ? `<a href="${r.link}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-download"></i></a>` : '—'}</td>
  `;
  tbody.appendChild(tr);
...
function updateDocumentoInventarioRow(r) {
  ...
  row.dataset.link = String(r.link || '');
  ...
  row.children[5].innerHTML = r.link ? `<a href="${r.link}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-download"></i></a>` : '—';
}
  ...
  if (barcoId && barcoId > 0) {
    fetch(`./api/inventario.php?action=listar_barco&barco_id=${encodeURIComponent(barcoId)}`)
      ...
      .then(json => {
        ...
        rows.forEach(r => {
          const opt = document.createElement('option');
          opt.value = String(r.inventario_id || r.id || '');
