...
function showCardMessage(container, type, text) {
    ...
    setTimeout(() => {
        div.style.opacity = '0';
        setTimeout(() => div.remove(), 6000); // 6s fade
    }, 100); // Inicia fade logo (mas o fade demora 6s, então fica visível por um tempo, diminuindo)
    // O usuário disse "ficando por uns 6 segundos e sumindo sutilmente".
    // Talvez ele queira que fique visível e DEPOIS suma.
    // Ajuste: Fica 4s visível, depois 2s fade out.
    div.style.transition = 'opacity 2s ease-out';
    setTimeout(() => {
        div.style.opacity = '0';
...
function showGlobalMessage(type, text) {
    ...
    if (statusText) {
        ...
        setTimeout(() => {
            statusText.style.transition = 'opacity 2s ease-out';
