...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const updateHeaderMeta = () => {
    ...
    headerMetaEl.textContent = `Local: ${localStr}, Vaga: ${vagaStr}, Cliente: ${clienteStr}, Modelo: ${modeloStr}`;
  };
  ...
  function renderDocumentos(docs) {
    ...
    docs.forEach(doc => {
      ...
      const downloadBtn = link ? `<a href="${link}" class="btn btn-sm btn-outline-primary" download aria-label="Download" title="Download"><i class="bi bi-download"></i></a>` : '<span class="text-muted">—</span>';
      tr.innerHTML = `
        <td style="padding-left: 20px;">${titulo}</td>
        <td style="padding-left: 20px;">${descricao}</td>
        <td>${tipo}</td>
        <td>${criado}</td>
        <td>
          <button type="button" class="btn btn-sm publico-toggle ${publicoBtnClass}" onclick="window.toggleDocumentoPublico(${Number(doc.id)}, ${publico})" aria-label="${publico ? 'Tornar privado' : 'Tornar público'}" title="${publico ? 'Documento público' : 'Documento privado'}">
            <i class="bi ${publicoIcon} ${publico ? 'publico-icon-public' : 'publico-icon-private'}"></i>
          </button>
        </td>
        <td>${downloadBtn}</td>
        <td class="text-end" style="padding-right: 20px;">
          <button type="button" class="btn btn-sm btn-outline-secondary me-2" data-bs-toggle="modal" data-bs-target="#modalEditDocumento" onclick="window.openEditDocumento(${Number(doc.id)})" aria-label="Editar" title="Editar">
            <i class="bi bi-pencil"></i>
          </button>
          <button type="button" class="btn btn-sm btn-outline-info me-2" data-bs-toggle="modal" data-bs-target="#modalShareDocumento" onclick="window.openShareDocumento(${Number(doc.id)})" aria-label="Compartilhar" title="Compartilhar">
