  ...
  function renderCategorias() {
    ...
    categorias.forEach((cat, ci) => {
      ...
      const controlsHeader = showControlesQualidade
        ? '<th class="qc-col">Controle de qualidade</th><th class="anexo-cell">Anexo</th>'
        : '<th class="realizado-col">Realizado em</th>';
      thead.innerHTML = `
      ...
      cat.servicos.forEach((svc, si) => {
        ...
        chk.addEventListener('change', () => { svc.concluido = chk.checked; updateCategoriaBadge(ci); });
      ...
      addAnchor.addEventListener('click', () => {
        cat.servicos.push({ id: cryptoRandomId(), nome: `Serviço ${cat.servicos.length+1}`, status: 'Pendente', concluido: false, cq: '', obs: '', anexo: null });
        renderCategorias();
      });
