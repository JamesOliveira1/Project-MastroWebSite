...
function updateUndoButtonState() {
  ...
  // Pulsar botão salvar quando houver alterações não salvas
  const saveBtn = document.getElementById('saveChangesBtn');
  if (saveBtn) {
    if (isDirty) saveBtn.classList.add('save-pulsing');
    else saveBtn.classList.remove('save-pulsing');
  }
}
  ...
  try {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (isEstaleiroPage) {
        ...
        // Limpar mapa de posições do localStorage ao carregar a página
        try {
          localStorage.removeItem('boatPositions');
          boatPositions = {};
          console.log('[estaleiro] boatPositions limpo no carregamento inicial');
        } catch (e) {
          console.warn('[estaleiro] falha ao limpar boatPositions', e);
        }
      ...
      if (refreshBtn) {
        ...
        refreshBtn.addEventListener('click', async function() {
          try {
            ...
            // Limpar mapa de posições antes de recarregar dados
            try {
              localStorage.removeItem('boatPositions');
              boatPositions = {};
              console.log('[estaleiro] boatPositions limpo ao atualizar dados');
            } catch (e) {
              console.warn('[estaleiro] falha ao limpar boatPositions no refresh', e);
            }
  ...
  (async () => {
    if (currentEditingBoat) {
      ...
      loadBoats();
      updateCounters();
      // Alterações em memória exigem salvar geral
      isDirty = true; updateUndoButtonState();
    } else {
      ...
      loadBoats();
      updateCounters();
      // Novos barcos criados em memória ainda não persistidos
      isDirty = true; updateUndoButtonState();
    }
  })();
}
      ...
      if (confirm('Tem certeza que deseja enviar este barco para a Lixeira?')) {
        (async () => {
          try {
            ...
            // Sinalizar alterações não salvas quando operação é somente local
            if (isTempId(currentEditingBoat.id)) { isDirty = true; updateUndoButtonState(); }
          } catch (e) {
    ...
    async function handleDrop(e) {
      ...
      if (completedIndex > -1) {
        try {
            ...
            if (!isTempId(boat.id)) {
              try {
                await moveBoatInDB(boat.id, localKeyToNome(boat.warehouse), numero);
              } catch (err) {
                console.error('Falha ao persistir retorno de Concluídos:', err);
              }
              await fetchOccupancyFromAPI();
              isDirty = false; updateUndoButtonState();
            } else {
              isDirty = true; updateUndoButtonState();
            }
            saveBoatsToStorage();
          ...
          try {
            ...
            if (!isTempId(boat.id)) {
              try {
                ...
                console.error('Falha ao persistir movimento de Pré-Projetos:', err);
              }
              await fetchOccupancyFromAPI();
              isDirty = false; updateUndoButtonState();
            } else {
        ...
        try {
          ...
          if (!isTempId(boat.id)) {
            try {
              ...
              console.error('Falha ao persistir movimento:', err);
            }
            await fetchOccupancyFromAPI();
            isDirty = false; updateUndoButtonState();
          } else {
...
function buildOccupancyPayload() {
  const map = new Map();
  const put = (b, localNome, numero) => {
    if (!b || b.id == null) return;
    const n = parseInt(String(b.id), 10);
    if (!Number.isFinite(n) || n <= 0) return;
    const rec = { barco_id: n, local_nome: localNome };
    if (typeof numero !== 'undefined' && numero !== null) rec.numero = parseInt(numero, 10);
    map.set(String(n), rec);
  };

  // Precedência correta: Galpões < Pré-Projetos < Concluídos < Lixeira
  // Começa com galpões e depois sobrepõe com locais especiais, garantindo uma única ocorrência final
  (boats || []).forEach(b => {
    if (b && b.warehouse && b.slot != null) {
      put(b, localKeyToNome(b.warehouse), b.slot);
    }
  });
  (preBoats || []).forEach(b => put(b, 'Pré-Projetos', null));
  (completedBoats || []).forEach(b => put(b, 'Concluídos', null));
  (trashBoats || []).forEach(b => put(b, 'Lixeira', null));
...
async function saveOccupancy() {
  ...
  try {
    ...
    await fetchOccupancyFromAPI();
    // Sincroniza mapa de posições com o estado vindo do backend para evitar sobrescrita indevida
    const newPosMap = {};
    (boats || []).forEach(b => {
      if (!b) return;
      if (isTempId(b.id)) return;
      if (b.warehouse && b.slot != null) {
        newPosMap[String(b.id)] = { warehouse: b.warehouse, slot: b.slot };
      }
    });
    savePositionsMap(newPosMap);
