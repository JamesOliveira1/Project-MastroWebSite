...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnAddSeries')?.addEventListener('click', () => openSeriesModal(null, ''));
    document.getElementById('btnSaveSeries')?.addEventListener('click', saveSeries);
    document.getElementById('btnDeleteSeries')?.addEventListener('click', deleteSeries);
    document.getElementById('btnOrganizarSeries')?.addEventListener('click', toggleSortSeries);
});
...
let currentSeriesModalInstance = null;
let currentSeriesModalType = null;
let seriesSortAsc = true;

function loadSeries() {
    togglePreloader(true);
    fetch('api/updatesite.php?action=list_series_items')
        .then(r => r.json())
        .then(d => {
            if (d.success) {
                allSeriesItems = d.data.slice().sort((a, b) => a.item.localeCompare(b.item));
                renderSeries(allSeriesItems);
            } else {
                document.getElementById('seriesGrid').innerHTML = '<tr><td class="text-danger" colspan="2">Erro ao carregar itens de série.</td></tr>';
            }
        })
        .catch(e => {
            document.getElementById('seriesGrid').innerHTML = '<tr><td class="text-danger" colspan="2">Erro de conexão.</td></tr>';
        })
        .finally(() => togglePreloader(false));
}

function renderSeries(items) {
    const body = document.getElementById('seriesGrid');
    body.innerHTML = '';
    items.forEach(it => {
        ...
        tdRelated.style.paddingLeft = '20px';
        tdRelated.style.width = '120px';
        if (it.related && Number(it.related) === 1) {
            const checkIcon = document.createElement('i');
            checkIcon.className = 'bi bi-check-lg';
            checkIcon.style.fontSize = '1.1rem';
            tdRelated.appendChild(checkIcon);
        } else {
            tdRelated.innerHTML = '';
        }
        const tdActions = document.createElement('td');
        tdActions.style.paddingLeft = '20px';
        tdActions.style.width = '120px';
        const btnEdit = document.createElement('button');
        btnEdit.className = 'btn btn-link btn-pencil p-0 me-3';
        btnEdit.title = 'Editar';
        btnEdit.innerHTML = '<i class="bi bi-pencil" style="font-size: 1.1rem;"></i>';
        btnEdit.onclick = () => openSeriesModal(it.id, it.item);
        const btnDel = document.createElement('button');
        btnDel.className = 'btn btn-link btn-xx p-0';
        btnDel.title = (it.related && Number(it.related) === 1) ? 'Não é possível excluir: item relacionado' : 'Excluir';
        btnDel.innerHTML = '<i class="bi bi-x-lg" style="font-size: 1.1rem;"></i>';
        if (it.related && Number(it.related) === 1) {
            btnDel.disabled = true;
        } else {
            btnDel.onclick = () => { currentSeriesId = it.id; deleteSeries(); };
        }
...
function openSeriesModal(id, name) {
    currentSeriesId = id || null;
    const input = document.getElementById('seriesName');
    input.value = name || '';
    const delBtn = document.getElementById('btnDeleteSeries');
    delBtn.style.display = currentSeriesId ? 'inline-block' : 'none';
    const modalEl = document.getElementById('modalEditSeries');
    currentSeriesModalInstance = null;
    currentSeriesModalType = null;
    if (typeof bootstrap !== 'undefined' && typeof bootstrap.Modal === 'function') {
        if (typeof bootstrap.Modal.getOrCreateInstance === 'function') {
            currentSeriesModalInstance = bootstrap.Modal.getOrCreateInstance(modalEl);
        } else if (typeof bootstrap.Modal.getInstance === 'function') {
            currentSeriesModalInstance = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
        } else {
            currentSeriesModalInstance = new bootstrap.Modal(modalEl);
        }
        currentSeriesModalType = 'bootstrap';
        currentSeriesModalInstance.show();
    } else if (window.jQuery && typeof jQuery.fn.modal === 'function') {
        currentSeriesModalType = 'jquery';
        jQuery(modalEl).modal('show');
        currentSeriesModalInstance = modalEl;
    } else {
        modalEl.classList.add('show');
        modalEl.style.display = 'block';
        document.body.classList.add('modal-open');
...
function closeSeriesModal() {
    const modalEl = document.getElementById('modalEditSeries');
    if (currentSeriesModalType === 'bootstrap' && currentSeriesModalInstance && typeof currentSeriesModalInstance.hide === 'function') {
        currentSeriesModalInstance.hide();
    } else if (currentSeriesModalType === 'jquery' && window.jQuery && typeof jQuery.fn.modal === 'function') {
        jQuery(modalEl).modal('hide');
    } else if (typeof bootstrap !== 'undefined' && typeof bootstrap.Modal === 'function') {
        const inst = (typeof bootstrap.Modal.getInstance === 'function' ? bootstrap.Modal.getInstance(modalEl) : null) || new bootstrap.Modal(modalEl);
        inst.hide();
...
function saveSeries() {
    ...
    if (!name) return;
    const payload = currentSeriesId ? { action: 'update_series_item', id: currentSeriesId, item: name } : { action: 'add_series_item', item: name };
    togglePreloader(true);
    fetch('api/updatesite.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(d => {
        if (d.success) {
            ...
            console.error('Erro ao salvar item de série:', d.message);
        }
    })
    .catch(e => {
        console.error('Erro ao salvar item de série:', e);
    })
    .finally(() => togglePreloader(false));
}
...
function deleteSeries() {
    ...
    if (!confirm('Tem certeza que deseja excluir o item de série?')) return;
    const payload = { action: 'delete_series_item', id: currentSeriesId };
    togglePreloader(true);
    fetch('api/updatesite.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(d => {
        if (d.success) {
            ...
            console.error('Erro ao excluir item de série:', d.message);
        }
    })
    .catch(e => {
        console.error('Erro ao excluir item de série:', e);
    })
    .finally(() => togglePreloader(false));
}

function toggleSortSeries() {
    seriesSortAsc = !seriesSortAsc;
    const btn = document.getElementById('btnOrganizarSeries');
    if (seriesSortAsc) {
        allSeriesItems.sort((a, b) => a.item.localeCompare(b.item));
        if (btn) btn.innerHTML = '<i class="bi bi-sort-alpha-down"></i> Organizar (Nome)';
    } else {
        allSeriesItems.sort((a, b) => b.item.localeCompare(a.item));
        if (btn) btn.innerHTML = '<i class="bi bi-sort-alpha-up"></i> Organizar (Nome)';
    }
    renderSeries(allSeriesItems);
}
