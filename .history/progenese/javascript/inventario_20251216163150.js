        ...
        ths.forEach(function(th) {
          th.addEventListener('click', function() {
            ...
            rows.sort(function(a, b) {
              ...
              if (aIsNum && bIsNum) { return dir === 'asc' ? na - nb : nb - na; }

              const ta = parseDate(va);
              const tb = parseDate(vb);
              if (ta != null && tb != null) { return dir === 'asc' ? ta - tb : tb - ta; }
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    const sel = document.getElementById('inventario-mes-select');
    if (sel) {
      sel.addEventListener('change', function(){ applyInventarioMonthFilter(); });
    }
    const agrupaBtn = document.getElementById('inventario-agrupa-btn');
    if (agrupaBtn) {
      agrupaBtn.addEventListener('click', function(){
        this.classList.toggle('active');
        this.setAttribute('aria-pressed', this.classList.contains('active') ? 'true' : 'false');
        if (isInventarioGroupingActive()) rebuildInventarioGrouping(); else clearInventarioGrouping();
      });
    }
  } catch (_) {}
  ...
  if (editModalEl) {
    editModalEl.addEventListener('shown.bs.modal', function() {
      ...
      normalizeQuantidadeInput(editModalEl.querySelector('#invQtd'));
    });
    editModalEl.addEventListener('hidden.bs.modal', function() {
      try { buildInventarioMonthOptions(); applyInventarioMonthFilter(); } catch (_) {}
    });
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', async function() {
        try {
          ...
          const quantidade = parseFloat(((document.getElementById('invQtd').value || '').trim()).replace(',', '.'));
          ...
          if (tds[2]) tds[2].textContent = formatQuantidade(updated.quantidade);
          if (tds[4]) tds[4].textContent = formatDateBR(updated.data || (document.getElementById('invData').value || ''));
...
function appendDocumentoInventarioRow(r) {
  ...
  tr.innerHTML = `
    ...
    <td class="text-center">${r.link ? `<a href="${r.link}" target="_blank" class="btn btn-sm btn_estaleirooff px30button d-inline-flex justify-content-center align-items-center mx-auto"><i class="bi bi-download"></i></a>` : '—'}</td>
  `;
  tbody.appendChild(tr);
...
function updateDocumentoInventarioRow(r) {
  ...
  row.children[5].className = 'text-center';
  row.children[5].innerHTML = newLink ? `<a href="${newLink}" target="_blank" class="btn btn-sm btn_estaleirooff px30button d-inline-flex justify-content-center align-items-center mx-auto"><i class="bi bi-download"></i></a>` : '—';
}
...
function getInventarioRows() {
  ...
  const tbody = table ? table.querySelector('tbody') : null;
  if (!tbody) return [];
  return Array.from(tbody.querySelectorAll('tr.inventario-row')).filter(function(row){ return !row.classList.contains('inventario-hidden'); });
}
...
function parseRowMonthKey(row) {
  const cell = row && row.children ? row.children[4] : null;
...
function isInventarioGroupingActive() {
  const btn = document.getElementById('inventario-agrupa-btn');
  return !!(btn && btn.classList.contains('active'));
}

function clearInventarioGrouping() {
  const tbody = document.querySelector('#inventario-table tbody');
  if (!tbody) return;
  tbody.querySelectorAll('tr.inventario-group-row').forEach(function(r){ r.remove(); });
  try { applyInventarioMonthFilter(); } catch (_) {}
}

function rebuildInventarioGrouping() {
  const tbody = document.querySelector('#inventario-table tbody');
  if (!tbody) return;
  // Remove grupos anteriores
  tbody.querySelectorAll('tr.inventario-group-row').forEach(function(r){ r.remove(); });
  // Captura linhas filtradas (visíveis pelo filtro, independente da paginação)
  const rows = Array.from(tbody.querySelectorAll('tr.inventario-row')).filter(function(r){ return !r.classList.contains('inventario-hidden'); });
  if (rows.length === 0) { renderInventarioPage(); updateInventarioPagination(); return; }
  const groups = new Map();
  rows.forEach(function(r){
    const tds = r.querySelectorAll('td');
    const codigo = tds[0] ? tds[0].innerText.trim() : '';
    const nome = tds[1] ? tds[1].innerText.trim() : '';
    const qtdStr = tds[2] ? tds[2].innerText.trim() : '0';
    const unidade = tds[3] ? tds[3].innerText.trim() : '—';
    const qtd = parseFloat(qtdStr.replace(',', '.'));
    if (!groups.has(codigo)) {
      groups.set(codigo, { codigo, nome, unidade, quantidade: (isNaN(qtd) ? 0 : qtd) });
    } else {
      const g = groups.get(codigo);
      g.quantidade += (isNaN(qtd) ? 0 : qtd);
    }
  });
  // Oculta todas as linhas originais durante agrupamento
  rows.forEach(function(r){ r.classList.add('inventario-hidden'); r.style.display = 'none'; });
  // Cria linhas de grupo
  groups.forEach(function(g){
    const tr = document.createElement('tr');
    tr.className = 'inventario-row inventario-group-row';
    tr.innerHTML = `
      <td style="padding-left:20px;">${g.codigo}</td>
      <td style="padding-left:20px;">${g.nome}</td>
      <td>${formatQuantidade(g.quantidade)}</td>
      <td>${g.unidade || '—'}</td>
      <td>—</td>
      <td>—</td>
      <td>—</td>
...
function buildInventarioMonthOptions() {
  const select = document.getElementById('inventario-mes-select');
  if (!select) return;
  const rows = Array.from(document.querySelectorAll('#inventario-table tbody tr.inventario-row'));
  const set = new Set();
  rows.forEach(function(r){ const k = parseRowMonthKey(r); if (k) set.add(k); });
  const keys = Array.from(set).sort(function(a,b){ return parseInt(a.replace('-','')) - parseInt(b.replace('-','')); });
  const prev = select.value;
  select.innerHTML = '';
  const optAtual = document.createElement('option'); optAtual.value = 'mes_atual'; optAtual.textContent = 'Mês atual'; select.appendChild(optAtual);
  ...
  keys.forEach(function(k){ const opt = document.createElement('option'); opt.value = k; opt.textContent = labelForMonthKey(k); select.appendChild(opt); });
  if (prev && Array.from(select.options).some(function(o){ return o.value === prev; })) { select.value = prev; } else { select.value = 'mes_atual'; }
}

function applyInventarioMonthFilter() {
  const select = document.getElementById('inventario-mes-select');
  const val = select ? select.value : 'mes_atual';
  const rows = Array.from(document.querySelectorAll('#inventario-table tbody tr.inventario-row'));
  const now = new Date();
  const currentKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  const targetKey = val === 'mes_atual' ? currentKey : (val === 'todos' ? null : val);
  rows.forEach(function(r){
    const k = parseRowMonthKey(r);
    const show = targetKey === null ? true : (k === targetKey);
    if (show) { r.classList.remove('inventario-hidden'); r.style.display = ''; }
    else { r.classList.add('inventario-hidden'); r.style.display = 'none'; }
  });
  inventarioOffset = 0;
  renderInventarioPage();
  updateInventarioPagination();
  try { if (isInventarioGroupingActive()) { rebuildInventarioGrouping(); } } catch (_) {}
}
...
function normalizeQuantidadeInput(el) {
  if (!el) return;
  const sanitize = (raw) => {
  ...
  el.addEventListener('blur', function() {
    const v = sanitize(this.value);
    if (v.endsWith(',')) this.value = v.slice(0, -1);
    else this.value = v;
  });
}
  ...
  try {
    ...
    rows.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td>${formatQuantidade(r.quantidade)}</td>
...
function setupAddInventarioModal() {
  ...
  if (modalEl && typeof bootstrap !== 'undefined') {
    modalEl.addEventListener('shown.bs.modal', function() {
      ...
      setToday();
      normalizeQuantidadeInput(qtdInput);
    });
  }
      ...
      try {
        ...
        if (tbody) {
          ...
          tr.innerHTML = `
            ...
            <td>${formatQuantidade(added.quantidade)}</td>
...
function showPedidoSendStatus(type, message) {
  ...
  if (qtdInput) normalizeQuantidadeInput(qtdInput);
