...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderDocumentos(docs) {
    ...
    docs.forEach(doc => {
      ...
      const downloadBtn = link ? `<a href="${link}" class="btn btn-sm btn_estaleirooff px30button" download aria-label="Download" title="Download"><i class="bi bi-download"></i></a>` : '<span class="text-muted">—</span>';
      tr.innerHTML = `
        ...
        <td>${downloadBtn}</td>
        <td class="text-end d-flex justify-content-end gap-2" style="padding-right: 20px;">
          <button  type="button" class="btn btn-sm btn_estaleirooff me-2 px30button" data-bs-toggle="modal" data-bs-target="#modalEditDocumento" onclick="window.openEditDocumento(${Number(doc.id)})" aria-label="Editar" title="Editar">
            <i class="bi bi-pencil"></i>
          </button>
          <button type="button" class="btn btn-sm btn_estaleirooff me-2 px30button" data-bs-toggle="modal" data-bs-target="#modalShareDocumento" onclick="window.openShareDocumento(${Number(doc.id)})" aria-label="Compartilhar" title="Compartilhar">
