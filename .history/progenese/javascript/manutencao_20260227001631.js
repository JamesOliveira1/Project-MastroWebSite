    ...
    if (btnNovo) {
        btnNovo.addEventListener('click', () => {
            ...
            if(btnNome) {
                ...
                btnNome.style.display = ''; // GARANTIR VISIBILIDADE
                btnNome.onclick = null; 
            }
            if(btnCodigo) {
                btnCodigo.innerHTML = '<i class="bi bi-search"></i>';
                btnCodigo.style.display = ''; // GARANTIR VISIBILIDADE
...
function setupBuscaEquipamentos() {
    ...
    const renderSearchResults = (items, offset, limit) => {
        ...
        resultsBody.innerHTML = items.map(item => {
            // Tratar campos nulos ou undefined e escapar caracteres especiais para HTML e JS inline
            const codigo = item.codigo_produto || '';
            const nome = item.nome || item.nome_item || '';
            
            // 1. Escapar barra invertida primeiro para não escapar as barras adicionadas depois
            // 2. Escapar aspas simples para a string JS dentro do onclick
            // 3. Escapar aspas duplas para o atributo HTML onclick
            const safeCodigo = codigo.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, "&quot;");
            const safeNome = nome.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, "&quot;");
    ...
    if(btnClear) {
        btnClear.addEventListener('click', () => {
            // Limpar apenas o corpo da tabela de resultados, mantendo cabeçalho e estrutura
            if(resultsBody) resultsBody.innerHTML = '';
            // Manter container visível, pois foi solicitado que a tabela pode ficar sempre visível
            // resultsContainer.style.display = 'none'; 
            // Mas se limpou a pesquisa, talvez queira esconder a tabela vazia ou mostrar msg "Faça uma busca"?
            // O usuário disse: "ele não deve limpar tudo, apenas os resultados... a tabela e cabeçalho... devem aparecer como aparecem inicialmente"
            // Inicialmente (no HTML) está display: block ou none?
            // No HTML original estava style="display: none;" mas mudamos para class="mb-3" (display block).
            // Então vamos manter visível mas vazio.
            
            // Esconder o botão limpar pois não há mais pesquisa ativa
            ...
            // Resetar botões de busca para estado inicial
            const btnNome = document.getElementById('btnBuscaEquipNome');
            const btnCodigo = document.getElementById('btnBuscaEquipCodigo');
            if(btnNome) {
                btnNome.innerHTML = '<i class="bi bi-search"></i>';
                btnNome.classList.remove('active');
            }
            if(btnCodigo) {
                btnCodigo.innerHTML = '<i class="bi bi-search"></i>';
...
function renderEquipamentos(openIds = []) {
    ...
    if (equipamentos.length === 0) {
        // Forçar limpeza do container antes de inserir mensagem
        container.innerHTML = '';
        const emptyMsg = document.createElement('div');
        emptyMsg.className = 'col-12 text-center py-5 w-100 text-muted';
        emptyMsg.textContent = 'Nenhum equipamento cadastrado.';
...
function editarEquipamento(id) {
    ...
    // Esconder botão limpar na edição
    const btnClear = document.getElementById('btnBuscaEquipClear');
    if(btnClear) btnClear.style.display = 'none';
...
async function excluirEquipamento() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Verificar se ficou vazio após exclusão e garantir mensagem
            const container = document.getElementById('equipamentosContainer');
            if (equipamentos.length === 0 && container) {
