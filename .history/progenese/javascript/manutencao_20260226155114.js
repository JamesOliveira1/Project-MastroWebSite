...
async function carregarEquipamentos(openId = null) {
...
function renderEquipamentos(openId = null) {
    const container = document.getElementById('equipamentosContainer');
    if (!container) return;
    ...
    equipamentos.forEach(equip => {
        ...
        // Se openId for fornecido e corresponder a este equipamento, adiciona a classe active
        const isActive = openId && equip.id == openId;
...
async function salvarEquipamento() {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (targetId) {
                // Não precisa mais de setTimeout, o render já cuida do active se passarmos o ID
                // Mas precisamos garantir que a lista seja renderizada com esse ID
                // carregarEquipamentos(targetId) já foi chamado acima?
                // Não, foi chamado await carregarEquipamentos(). Precisamos ajustar.
                // Mas espera, carregarEquipamentos é chamado acima sem argumentos.
                // Vamos chamar novamente? Não, ineficiente.
                // Melhor: mudar a chamada acima.
            }
        } else {
            alert('Erro ao salvar: ' + (result.error || 'Erro desconhecido'));
        }
        ...
        if (result.success) {
             ...
             if (equipId) {
                 setTimeout(() => {
                     ...
                     if (newItem) toggleDetails(equipId);
                 }, 100);
             }
        } else {
            alert('Erro ao excluir: ' + (result.error || 'Erro desconhecido'));
    ...
    try {
        ...
        if (result.success) {
            ...
            if (modalInstance) modalInstance.hide();
             ...
             if (equipId) {
                 setTimeout(() => {
                     // Após recarregar, o item nasce fechado. toggleDetails(id) abre.
                     const details = document.getElementById(`details-${equipId}`);
                     if (details) {
                         toggleDetails(equipId);
                         // Scroll
                         details.closest('.equipment-item').scrollIntoView({ behavior: 'smooth', block: 'center' });
                     }
                 }, 150);
             }
        } else {
