            ...
            <div class="flip-container" id="${cardId}" onclick="toggleFlip('${cardId}')">
                <div class="flipper">
                    ...
                    <div class="back">
                        ...
                        <div class="back-footer" onclick="abrirModalEdicao(${molde.id}, event)" onmouseover="this.style.color='#0071d2'" onmouseout="this.style.color='#5c8eb0'">
...
function abrirModalEdicao(id, event) {
    ...
    // Ocultar preview no modal de edição inicialmente (só mostra se selecionar nova imagem)
    const preview = document.getElementById('previewImagem');
    preview.style.display = 'none';
    preview.src = '';
...
async function salvarMolde() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Mostrar mensagem de sucesso (se houver elemento na página)
            const statusText = document.getElementById('status-text');
            if (statusText) {
                statusText.className = 'text-success fw-bold';
                statusText.textContent = result.message || 'Operação realizada com sucesso!';
                setTimeout(() => {
                    statusText.textContent = '';
                }, 3000);
            }

            // Se for edição, manter o card virado (opcional - mas como recarrega tudo, perde o estado.
            // Para manter virado, precisamos saber qual ID foi editado e virar ele após renderizar.
            if (formData.get('id')) {
                 const editedId = formData.get('id');
                 // Pequeno delay para garantir que renderizou
                 setTimeout(() => {
                     const card = document.getElementById(`card-${editedId}`);
