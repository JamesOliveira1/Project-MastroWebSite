                        ...
                        <h4>
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <button class="btn btn-sm btn-outline-primary" onclick="addManutencao(${equip.id}, event)">
                                    <i class="bi bi-wrench-adjustable"></i> Manutenção
                                </button>
                            </div>
                        </h4>
...
function salvarEquipamento() {
    ...
    const novoEquip = {
        ...
        nota_fiscal: document.getElementById('equipNotaFiscal').value,
...
function alterarStatus(id, event) {
    ...
    // Manter o item aberto após re-renderizar - sem animação de fechamento/abertura
    setTimeout(() => {
        const details = document.getElementById(`details-${id}`);
        if(details) {
            const item = details.closest('.equipment-item');
            if(item) {
                // Remove transição temporariamente para evitar "piscar"
                details.style.transition = 'none';
                item.classList.add('active');
                
                // Define altura exata imediatamente
                details.style.maxHeight = 'none'; // Deixa o conteúdo definir a altura
                const scrollHeight = details.scrollHeight;
                details.style.maxHeight = (scrollHeight + 50) + 'px';
                
                // Restaura transição após renderização
                setTimeout(() => {
