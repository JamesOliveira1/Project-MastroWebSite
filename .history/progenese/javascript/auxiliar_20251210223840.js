...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderComplementaryInfo(infos) {
    ...
    infos.forEach(info => {
      ...
      row.innerHTML = `
        ...
        <td class="text-end">
          <button type="button" class="btn btn-outline-danger btn-sm" onclick="deleteComplementaryInfo(${info.id})" aria-label="Excluir" title="Excluir">
  ...
  if (duplicateOpcionalBtn) {
    duplicateOpcionalBtn.addEventListener('click', async () => {
      try {
        ...
        closeModalGeneric('modalEditOpcional');
        alert('Opcional duplicado com sucesso.');
        await loadOpcionais(boatId);
      } catch (err) {
  ...
  function renderDocumentos(docs) {
    ...
    docs.forEach(doc => {
      ...
      const downloadBtn = link ? `<a href="${link}" class="btn btn-sm btn_estaleirooff px30button d-inline-flex justify-content-center align-items-center mx-auto" download aria-label="Download" title="Download"><i class="bi bi-download"></i></a>` : '<span class="text-muted">—</span>';
      tr.innerHTML = `
        ...
        <td style="text-align: center;">
          <button type="button" class="btn btn-sm btn-outline-warning publico-toggle ${publicoBtnClass} px30button" onclick="window.toggleDocumentoPublico(${Number(doc.id)}, ${publico})" aria-label="${publico ? 'Tornar privado' : 'Tornar público'}" title="${publico ? 'Documento público' : 'Documento privado'}">
            <i class="bi ${publicoIcon} ${publico ? 'publico-icon-public' : 'publico-icon-private'}"></i>
          </button>
        </td>
        <td style="text-align: center;">${downloadBtn}</td>
        <td style="text-align: center;">
          <div class="doc-actions d-inline-flex justify-content-center align-items-center gap-2">
            <button type="button" class="btn btn-sm btn_estaleirooff px30button" data-bs-toggle="modal" data-bs-target="#modalEditDocumento" onclick="window.openEditDocumento(${Number(doc.id)})" aria-label="Editar" title="Editar">
              <i class="bi bi-pencil"></i>
            </button>
            <button type="button" class="btn btn-sm btn_estaleirooff px30button" data-bs-toggle="modal" data-bs-target="#modalShareDocumento" onclick="window.openShareDocumento(${Number(doc.id)})" aria-label="Compartilhar" title="Compartilhar">
              <i class="bi bi-share"></i>
            </button>
            <button type="button" class="btn btn-sm btn-outline-danger px30button" data-bs-toggle="modal" data-bs-target="#modalDeleteDocumento" onclick="window.openDeleteDocumento(${Number(doc.id)})" aria-label="Excluir" title="Excluir">
              <i class="bi bi-trash"></i>
            </button>
          </div>
        </td>
