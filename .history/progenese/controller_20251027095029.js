...
let boatPositions = JSON.parse(localStorage.getItem('boatPositions')) || {}; // Posições dos barcos
const BOATS_API = 'api/barcos.php'; // API de Barcos
...
/**
 * Cria ou edita um barco no estado local (persistência no saveOccupancy).
 */
function saveBoat() {
...
// A função applyPositionsFromStorage não foi encontrada no código original.
// Mantendo a chamada no DOMContentLoaded para evitar quebra, mas a função não está definida.
// Se ela for crítica, o usuário deve ser notificado.
function applyPositionsFromStorage() {
  // Função não implementada no código original.
}