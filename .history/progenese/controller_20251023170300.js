///////////////////////////     estaleiro         ///////////////////////////
    ...
    function initializeSystem() {
      ...
      document.querySelectorAll('#boatModal [data-dismiss="modal"], #boatModal [data-bs-dismiss="modal"], #closeBoatModalBtn').forEach(btn => {
    ...
    function selectBoat(boatId) {
      const boat = boats.find(b => b.id === boatId) || preBoats.find(b => b.id === boatId);
      if (!boat) return;
      ...
      // Add selection to current boat (apenas se existir no grid)
        ...
        if (completedIndex > -1) {
          (async () => {
            try {
              ...
              boat.warehouse = slot.dataset.warehouse; boat.slot = numero; delete boat.completedDate;
              boats.push(boat);
