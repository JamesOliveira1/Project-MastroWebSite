      ...
      (async () => {
        if (currentEditingBoat) {
          try {
            ...
            await apiRequest('editar_barco', { id: idNum, numero_serie: codigo, cliente_nome: cliente, modelo: modelo, status_producao: progress });
            currentEditingBoat.codigo = codigo; currentEditingBoat.modelo = modelo; currentEditingBoat.cliente = cliente; currentEditingBoat.progress = progress;
          } catch (e) { alert('Falha ao editar barco: ' + e.message); return; }
        } else {
          ...
          if (modal.dataset.createInPreProject === 'true') {
            try {
              ...
              const newPreBoat = { id: String(created.id), codigo: created.numero_serie || codigo, modelo: created.modelo, cliente: created.cliente_nome, progress };
              ...
              return;
            } catch (e) { alert('Falha ao criar barco em Pré-Projetos: ' + e.message); return; }
          }
          ...
          if (preselectedWarehouse && preselectedSlot) {
            ...
            if (targetSlot && !targetSlot.dataset.boatId) { warehouse = preselectedWarehouse; slot = preselectedSlot; }
          }
          if (!warehouse) {
          ...
          try {
            ...
            boats.push(newBoat);
          } catch (e) { alert('Falha ao criar/alocar barco: ' + e.message); return; }
        }
        loadBoats(); updateCounters(); hideModal();
