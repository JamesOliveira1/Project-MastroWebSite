  ...
  populateProductionDropdowns() {
    ...
    Promise.all([
      ...
      const lamList = (occ || []).filter(row => {
        ...
        const an = parseInt(a.numero, 10) || 0;
        const bn = parseInt(b.numero, 10) || 0;
        return an - bn;
      });
      ...
      if (lamUl) {
        if (lamList.length === 0) {
          ...
          spanEmpty.textContent = 'Sem barcos na laminação';
      ...
      if (montUl) {
        if (montList.length === 0) {
          const liEmpty = document.createElement('li');
          const spanEmpty = document.createElement('span');
          spanEmpty.textContent = 'Sem barcos na montagem';
          liEmpty.appendChild(spanEmpty);
