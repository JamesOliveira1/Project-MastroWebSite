...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'list_motors') {
            ...
            $sql = "SELECT s.*, EXISTS(SELECT 1 FROM rel_barco_itens_serie r WHERE r.item_serie_id = s.id) AS related
                    FROM site_itens_de_serie s
                    ORDER BY s.item";
            $items = queryAll($sql);
            ...
            foreach ($boats as $boat) {
                ...
                // Opcionais
                $optionsSql = "
                    SELECT o.id, o.opcional as name, o.valor as price
                ...
                // Motores
                $powersSql = "
                    SELECT m.id, m.motor as name, m.valor as motorPrice
                    FROM site_motor m
                ...
                if (!$hasNoMotor) {
                    $powersRows[] = ['id' => 0, 'name' => 'Sem motor', 'motorPrice' => 0];
                }
                ...
                $boatOptions[$model] = [
                    ...
                    // Dados técnicos para o proposal.js
                    'categoria' => $boat['categoria'],
                    'comprimento' => $boat['comprimento_m'],
                    'largura' => $boat['largura_m'],
                    'calado' => $boat['calado_m'],
                    'peso' => $boat['peso_kg'],
                    'agua' => $boat['agua_l'],
                    'combustivel' => $boat['combustivel_l'],
                    'motorizacao' => $boat['motorizacao'],
                    'passageiros' => $boat['passageiros'],
                    'pernoite' => $boat['pernoite'],
                    'velocidade_cruzeiro' => $boat['velocidade_cruzeiro_nos'],
                    'autonomia' => $boat['autonomia_milhas'],
                    'velocidade_maxima' => $boat['velocidade_maxima_nos']
                ];
            }
        ...
        if ($action === 'update_motor') {
            ...
            logActivity('Atualizou motor', 'Motor', (int)$id, null, "Atualizou motor '$motorName' para valor R$ " . number_format((float)$valor, 2, ',', '.'));
            ...
            logActivity('Atualizou opcional', 'Opcional', (int)$id, null, "Atualizou opcional '$name' para valor R$ " . number_format((float)$valor, 2, ',', '.'));
            ...
            logActivity('Atualizou opcional', 'Opcional', (int)$id, null, "Atualizou opcional '$name' (com imagem)");
            ...
            logActivity('Excluiu opcional', 'Opcional', (int)$id, null, "Excluiu opcional ID $id");
            echo json_encode(['success' => true]);
            exit;
        } elseif ($action === 'add_series_item') {
            $name = trim((string)($input['item'] ?? ''));
            if ($name === '') {
                echo json_encode(['success' => false, 'message' => 'Nome do item é obrigatório']);
                exit;
            }
            execute("INSERT INTO site_itens_de_serie (item) VALUES (:item)", [':item' => $name]);
            $newId = getConnection()->lastInsertId();
            logActivity('Adicionou item de série', 'ItemSerie', (int)$newId, null, "Adicionou item de série '$name'");
            echo json_encode(['success' => true, 'id' => (int)$newId]);
            exit;
        } elseif ($action === 'update_series_item') {
            $id = $input['id'] ?? null;
            $name = trim((string)($input['item'] ?? ''));
            if (!$id || $name === '') {
            ...
            execute("UPDATE site_itens_de_serie SET item = :item WHERE id = :id", [':item' => $name, ':id' => $id]);
            logActivity('Atualizou item de série', 'ItemSerie', (int)$id, null, "Atualizou item de série para '$name'");
            echo json_encode(['success' => true]);
            exit;
        } elseif ($action === 'delete_series_item') {
            $id = $input['id'] ?? null;
            if (!$id) {
                echo json_encode(['success' => false, 'message' => 'ID do item não fornecido']);
                exit;
            }
            $related = queryOne("SELECT COUNT(*) AS c FROM rel_barco_itens_serie WHERE item_serie_id = :id", [':id' => $id]);
            $count = isset($related['c']) ? (int)$related['c'] : 0;
            if ($count > 0) {
                echo json_encode(['success' => false, 'message' => 'Este item está relacionado a um ou mais modelos e não pode ser excluído.']);
                exit;
            }
            execute("DELETE FROM site_itens_de_serie WHERE id = :id", [':id' => $id]);
            logActivity('Excluiu item de série', 'ItemSerie', (int)$id, null, "Excluiu item de série ID $id");
            echo json_encode(['success' => true]);
            exit;
            ...
            $boatName = queryOne("SELECT modelo FROM site_barco WHERE id = :id", [':id' => $barcoId])['modelo'] ?? 'ID ' . $barcoId;
            logActivity('Relacionou itens', 'Barco', (int)$barcoId, (int)$barcoId, "Relacionou itens no modelo '$boatName'");
            ...
            if ($existing) {
                ...
                logActivity('Atualizou opcional', 'Opcional', (int)$id, null, "Atualizou opcional '$name' (upload de imagem)");
                echo json_encode(['success' => true, 'message' => 'Opcional atualizado']);
                exit;
            } else {
                ...
                $newId = getConnection()->lastInsertId();
                logActivity('Adicionou opcional', 'Opcional', (int)$newId, null, "Adicionou opcional '$name'");
        ...
        if ($action === 'update_model') {
            ...
            logActivity('Editou modelo', 'Barco', (int)$id, (int)$id, "Editou ficha técnica do modelo '$modelo'");
