...
try {
  ...
  switch ($action) {
    ...
    case 'adicionar': {
      ...
      try {
        if (isset($_FILES['arquivo'])) {
          $link = handleUpload($_FILES['arquivo']);
        } else {
    ...
    case 'editar': {
      ...
      $existing = queryOne('SELECT id, link FROM documentos WHERE id = :id', [':id' => $id]);
      ...
      try {
        if (isset($_FILES['arquivo'])) {
          $newLink = handleUpload($_FILES['arquivo']);
        }
      } catch (Throwable $e) {
        jsonResponse(['ok' => false, 'error' => 'Upload inválido: ' . $e->getMessage()], 400);
      }
    ...
    case 'excluir': {
      ...
      if (!empty($doc['link'])) {
        ...
        // Caminho físico correspondente
        if (strpos($link, '../assets/docs/') !== false) {
          $filename = basename($link);
          $path = __DIR__ . '/../../assets/docs/' . $filename;
          if (is_file($path)) {
            @unlink($path);
          }
        }
