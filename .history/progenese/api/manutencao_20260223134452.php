...
function handleSave($pdo) {
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['imagem']['tmp_name'], $destino)) {
            // Caminho relativo para salvar no banco (acessível pelo navegador)
            // O frontend está em progenese/moldes.html, então o path relativo da imagem deve ser ../assets/img/moldes/...
    ...
    if ($id) {
        ...
        echo json_encode(['success' => true, 'message' => 'Molde criado com sucesso!', 'id' => $pdo->lastInsertId()]);
    }
}
...
function handleDelete($pdo) {
    ...
    $molde = queryOne("SELECT imagem FROM moldes WHERE id = ?", [$id]);
    if ($molde && $molde['imagem'] && strpos($molde['imagem'], 'semfoto.png') === false) {
