...
try {
  ...
  switch ($action) {
    ...
    case 'excluir': {
      ...
      try {
        $boatIdRow = queryOne('SELECT barco_id FROM opcionais WHERE id = :id', [':id' => $id]);
        $barcoId = (int)($boatIdRow['barco_id'] ?? 0);
        $boat = $barcoId ? queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $barcoId]) : null;
        $desc = 'excluiu opcional #' . $id . ' em barco #' . $barcoId . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
