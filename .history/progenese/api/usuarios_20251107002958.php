...
try {
  ...
  switch ($action) {
    ...
    case 'login': {
      ...
      jsonResponse(['ok' => true, 'redirect' => '/progenese/estaleiro.html', 'email' => (string)($user['email'] ?? '')]);
    }
    ...
    case 'session': {
      ...
      jsonResponse(['ok' => true, 'authenticated' => true, 'usuario' => $_SESSION['usuario'] ?? '', 'email' => $_SESSION['email'] ?? '', 'cargo' => $_SESSION['cargo'] ?? '']);
    }
