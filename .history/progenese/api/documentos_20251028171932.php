...
function handleUpload(?array $file, int $barco_id): ?string {
  ...
  $docsDir = $uploadDir . '/docs/' . $barco_id;
  if (!is_dir($docsDir)) {
  ...
  // Link relativo usado pelo front (a partir de progenese/*.html)
  $publicLink = '../assets/docs/' . $barco_id . '/' . $final;
  return $publicLink;
}
...
try {
  ...
  switch ($action) {
    case 'listar': {
      ...
      $rows = queryAll('SELECT id, barco_id, titulo, descricao, tipo, link, criado_em FROM documentos WHERE barco_id = :barco_id ORDER BY id DESC', [':barco_id' => $barco_id]);
    ...
    case 'excluir': {
      ...
      // Tenta remover arquivo físico se estiver em assets/docs
      if (!empty($doc['link'])) {
        $link = (string)$doc['link'];
        // Caminho físico correspondente (suporta subpastas por barco)
        $prefix = '../assets/docs/';
        if (strpos($link, $prefix) !== false) {
          $relative = substr($link, strlen($prefix)); // e.g. "123/arquivo.pdf" ou apenas "arquivo.pdf"
          $path = __DIR__ . '/../../assets/docs/' . $relative;
          if (!is_file($path)) {
            // Fallback para raiz se fosse antigo
            $path = __DIR__ . '/../../assets/docs/' . basename($relative);
          }
          if (is_file($path)) {
            @unlink($path);
          }
