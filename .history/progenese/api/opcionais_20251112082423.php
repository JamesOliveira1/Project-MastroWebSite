...
try {
  ...
  switch ($action) {
    ...
    case 'adicionar': {
      ...
      // Log de atividade: adicionar opcional
      try {
        $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => (int)$row['barco_id']]);
        $desc = 'adicionou opcional "' . (string)($row['nome'] ?? '') . '" em barco #' . (int)$row['barco_id'] . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('adicionar', 'opcionais', (int)$row['id'], (int)$row['barco_id'], $desc);
      } catch (Throwable $_) {}
    ...
    case 'editar': {
      ...
      $row = queryOne('SELECT id, barco_id, nome, descricao, quantidade, instalado, observacao FROM opcionais WHERE id = :id', [':id' => $id]);
      // Log de atividade: editar opcional
      try {
        $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => (int)$row['barco_id']]);
        $desc = 'editou opcional "' . (string)($row['nome'] ?? '') . '" em barco #' . (int)$row['barco_id'] . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('editar', 'opcionais', (int)$row['id'], (int)$row['barco_id'], $desc);
      } catch (Throwable $_) {}
      jsonResponse(['ok' => true, 'data' => $row]);
    }
    ...
    case 'excluir': {
      $id = (int)($_POST['id'] ?? ($input['id'] ?? 0));
      if ($id <= 0) jsonResponse(['ok' => false, 'error' => 'ID inválido'], 400);
      $opt = queryOne('SELECT id, barco_id, nome FROM opcionais WHERE id = :id', [':id' => $id]);
      ...
      // Log de atividade: excluir opcional
      try {
        $barcoId = (int)($opt['barco_id'] ?? 0);
        $boat = $barcoId ? queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => $barcoId]) : null;
        $desc = 'excluiu opcional "' . (string)($opt['nome'] ?? '') . '" em barco #' . $barcoId . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('excluir', 'opcionais', $id, $barcoId, $desc);
      } catch (Throwable $_) {}
