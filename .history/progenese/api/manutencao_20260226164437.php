...
function handleDeleteHistory($pdo) {
    ...
    $hist = queryOne("SELECT os, equipamento_id FROM historico_manutencao WHERE id = ?", [$id]);
    if ($hist && $hist['os']) {
        $osPath = $hist['os'];
        if (strpos($osPath, 'docs/') === 0) {
             $arquivoFisico = __DIR__ . '/../' . $osPath;
    ...
    if ($hist) {
        logActivity('excluir', 'historico_manutencao', (int)$hist['equipamento_id'], null, "Excluiu manutenção ID $id");
    }
