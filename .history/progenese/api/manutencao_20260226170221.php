...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'search_inventory') {
            ...
            // Buscar em itens_inventario por item ou codigo_produto
            $sql = "SELECT codigo_produto as codigo, item, unidade FROM itens_inventario 
                    WHERE item LIKE ? OR codigo_produto LIKE ? 
                    LIMIT 20";
            $results = queryAll($sql, ["%$term%", "%$term%"]);
...
function handleSave($pdo) {
    ...
    // Se enviou arquivo mas veio vazio (ex: fake file do JS), ignora e mantem o antigo
    // O else aqui não é necessário, pois $imagemPath já tem o valor antigo recuperado acima.
...
function handleSaveHistory($pdo) {
    ...
    // Recuperar OS antiga se for edição
    $osPath = null;
    if ($id) {
        $oldHist = queryOne("SELECT os FROM historico_manutencao WHERE id = ?", [$id]);
        if ($oldHist) {
            $osPath = $oldHist['os'];
        }
    }

    // Verificar remoção de OS
    $removerOS = isset($_POST['remover_os']) && $_POST['remover_os'] == '1';
    if ($removerOS && $osPath) {
        $pathRelativoApi = str_replace('../docs', '../../docs', $osPath);
        // Ajuste path relativo: docs/OS/file.pdf -> ../docs/OS/file.pdf a partir da API
        // O path salvo no banco é "docs/OS/..." (ver alteração anterior)
        // Se estiver "docs/OS/...", então da API é "../docs/OS/..."
        
        // Verifica se o path começa com docs/
        if (strpos($osPath, 'docs/') === 0) {
             $arquivoFisico = __DIR__ . '/../' . $osPath;
        } else {
             // Compatibilidade com antigo ../assets
             $arquivoFisico = __DIR__ . '/' . str_replace('../assets', '../../assets', $osPath);
        }
    ...
    if (isset($_FILES['os_file']) && $_FILES['os_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['os_file']['tmp_name'], $destino)) {
            // Remover antigo se existir e não tiver sido removido explicitamente (substituição)
            if ($osPath && !$removerOS) {
                 if (strpos($osPath, 'docs/') === 0) {
                     $oldFile = __DIR__ . '/../' . $osPath;
                 } else {
                     $oldFile = __DIR__ . '/' . str_replace('../assets', '../../assets', $osPath);
                 }
    ...
    // Buscar nome do equipamento para log
    $equip = queryOne("SELECT nome FROM equipamento_manutencao WHERE id = ?", [$equipamento_id]);
    $nomeEquip = $equip ? $equip['nome'] : 'Equipamento';

    if ($id) {
        // UPDATE
        $sql = "UPDATE historico_manutencao SET 
            data_inicial = ?, data_retorno = ?, tipo = ?, 
            observacao = ?, responsavel = ?, os = ?, is_concluida = ?
            WHERE id = ?";
        execute($sql, [
            $data_inicial, $data_retorno, $tipo,
            $observacao, $responsavel, $osPath, $is_concluida, $id
        ]);
        $msg = 'Histórico atualizado!';
        logActivity('editar', 'historico_manutencao', (int)$equipamento_id, null, "Editou manutenção em $nomeEquip ($tipo)");
    } else {
        // INSERT
        ...
        logActivity('criar', 'historico_manutencao', (int)$equipamento_id, null, "Adicionou manutenção em $nomeEquip ($tipo)");
    }
 ...
 */
function handleDeleteHistory($pdo) {
    $id = $_POST['id'] ?? '';
    if (!$id) {
    ...
    // Buscar arquivo para deletar e nome do equipamento para log
    $hist = queryOne("SELECT hm.os, hm.equipamento_id, em.nome as nome_equipamento 
                      FROM historico_manutencao hm
                      JOIN equipamento_manutencao em ON hm.equipamento_id = em.id 
                      WHERE hm.id = ?", [$id]);
    ...
    execute("DELETE FROM historico_manutencao WHERE id = ?", [$id]);
    
    if ($hist) {
        $nomeEquip = $hist['nome_equipamento'] ?? 'Equipamento';
        logActivity('excluir', 'historico_manutencao', (int)$hist['equipamento_id'], null, "Excluiu manutenção em $nomeEquip");
    }

    echo json_encode(['success' => true, 'message' => 'Histórico excluído!']);
}
