...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'list_equip') {
            // Buscar equipamentos
            $equips = queryAll("SELECT * FROM equipamento_manutencao ORDER BY id DESC");
            
            // Para cada equipamento, buscar histórico (otimização futura: JOIN ou single query)
            // Como volume é pequeno, loop é aceitável por enquanto
...
function handleSaveEquip($pdo) {
    ...
    // O JS usa 'ferramentas' no placeholder. Vamos criar 'equipamentos'.
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
        $uploadDir = __DIR__ . '/../../assets/img/equipamentos/';
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0777, true);
        }
        ...
        if (move_uploaded_file($_FILES['imagem']['tmp_name'], $destino)) {
            ...
            $imagemPath = '../assets/img/equipamentos/' . $novoNome;
        }
    }
