...
try {
  ...
  switch ($action) {
    ...
    case 'exportar_excel_inventario': {
      ...
      header('Content-Disposition: attachment;filename="inventario_barco_'.$barcoId.'.xls"');
    ...
    case 'exportar_excel_documento_inventario': {
      $barcoId = (int)($_GET['barco_id'] ?? $_POST['barco_id'] ?? ($input['barco_id'] ?? 0));
      if ($barcoId <= 0) {
        if (function_exists('header_remove')) { header_remove('Content-Type'); }
        header('Content-Type: text/plain; charset=utf-8');
      ...
      $rows = queryAll('SELECT di.titulo, di.observacao, di.tipo, di.criado_em, di.link, ib.nome_item AS associado_nome
      ...
      if (function_exists('header_remove')) { header_remove('Content-Type'); }
      header('Content-Type: application/vnd.ms-excel');
      header('Content-Disposition: attachment;filename="documentos_inventario_barco_'.$barcoId.'.xls"');
      header('Cache-Control: max-age=0');
      ...
      $boatClient = htmlspecialchars((string)($existsBoat['cliente_nome'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
      echo "<table>";
      echo "<caption>Documentos do Inventário — Barco #$barcoId — $boatTitle — $boatClient</caption>";
      echo "<tr><th>Título</th><th>Item do inventário</th><th>Observação</th><th>Tipo</th><th>Data</th><th>Arquivo</th></tr>";
      ...
      if (!$rows || count($rows) === 0) {
        ...
        foreach ($rows as $r) {
          $titulo = htmlspecialchars((string)($r['titulo'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          $associado = htmlspecialchars((string)($r['associado_nome'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          $obs = htmlspecialchars((string)($r['observacao'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          $tipo = htmlspecialchars((string)($r['tipo'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          $data = htmlspecialchars((string)($r['criado_em'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          $link = htmlspecialchars((string)($r['link'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
          echo "<tr>";
          echo "<td>$titulo</td>";
          echo "<td>" . ($associado !== '' ? $associado : '—') . "</td>";
          echo "<td>" . ($obs !== '' ? $obs : '—') . "</td>";
