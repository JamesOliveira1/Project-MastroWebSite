...
try {
  ...
  switch ($action) {
    ...
    case 'adicionar': {
      ...
      // Log de atividade: adicionar documento
      try {
        $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => (int)$row['barco_id']]);
        $desc = 'adicionou documento "' . (string)($row['titulo'] ?? '') . '" em barco #' . (int)$row['barco_id'] . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('adicionar', 'documentos', (int)$row['id'], (int)$row['barco_id'], $desc);
      } catch (Throwable $_) {}
    ...
    case 'editar': {
      ...
      // Log de atividade: editar documento
      try {
        $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => (int)$row['barco_id']]);
        $desc = 'editou documento "' . (string)($row['titulo'] ?? '') . '" em barco #' . (int)$row['barco_id'] . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('editar', 'documentos', (int)$row['id'], (int)$row['barco_id'], $desc);
      } catch (Throwable $_) {}
      jsonResponse(['ok' => true, 'data' => $row]);
    }
    ...
    case 'excluir': {
      ...
      // Log de atividade: excluir documento
      try {
        $boat = queryOne('SELECT cliente_nome, modelo FROM barcos WHERE id = :id', [':id' => (int)$documento['barco_id']]);
        $desc = 'excluiu documento #' . $id . ' em barco #' . (int)$documento['barco_id'] . ' — ' . (string)($boat['modelo'] ?? '') . ' — ' . (string)($boat['cliente_nome'] ?? '');
        logActivity('excluir', 'documentos', $id, (int)$documento['barco_id'], $desc);
      } catch (Throwable $_) {}
