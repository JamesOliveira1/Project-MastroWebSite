...
function ensureDocumentoInventarioTable(PDO $sqlite): void {
  $sqlite->exec('CREATE TABLE IF NOT EXISTS documento_inventario (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  $sqlite->exec('CREATE INDEX IF NOT EXISTS idx_documento_inventario_barco ON documento_inventario(barco_id)');
  $sqlite->exec('CREATE INDEX IF NOT EXISTS idx_documento_inventario_item ON documento_inventario(inventario_barco_id)');
}
...
try {
  ...
  switch ($action) {
    ...
    case 'listar_documentos_inventario': {
      ...
      if ($barcoId <= 0) { jsonResponse(['ok' => false, 'error' => 'barco_id inválido'], 400); }
      $sqlite = getConnection();
      ensureDocumentoInventarioTable($sqlite);
