///////////////////////////     estaleiro         ///////////////////////////
    ...
    // UI modal helpers (fallback se Bootstrap JS não estiver ativo)
    function hideModal(modalIdOrEvent) {
      const modalId = typeof modalIdOrEvent === 'string' ? modalIdOrEvent : 'boatModal';
      const modalEl = document.getElementById(modalId) || document.querySelector('.modal.show') || document.querySelector('.modal');
      if (!modalEl) return;
    ...
    function showModal(modalIdOrEvent) {
      const modalId = typeof modalIdOrEvent === 'string' ? modalIdOrEvent : 'boatModal';
...
function renderCompletedBoats() {
   ...
   if (!container) return;
   container.innerHTML = '';
   const list = completedBoats || [];
   if (!list.length) {
     const p = document.createElement('p');
     p.textContent = 'Nenhum projeto concluído ainda';
...
function loadBoats() {
  try {
    // Reset visual state for all slots
    document.querySelectorAll('.boat-slot').forEach(slot => {
      slot.classList.remove('occupied');
      delete slot.dataset.boatId;
    ...
    // Render boats into their slots
    (boats || []).forEach(b => {
      if (!b || !b.warehouse || b.slot == null) return;
      const slotEl = document.querySelector(`.boat-slot[data-warehouse='${b.warehouse}'][data-slot='${b.slot}']`);
      if (!slotEl) return;
      slotEl.dataset.boatId = b.id;
      slotEl.classList.add('occupied');

      const content = slotEl.querySelector('.slot-content');
      if (content) {
        content.innerHTML = `
          <div class='boat-info' draggable='true' data-boat-id='${b.id}'>
            <i class='bi bi-anchor-fill boat-anchor'></i>
            <div class='boat-model'>${b.modelo || 'Modelo N/A'}</div>
            <div class='boat-code'>${b.codigo || 'Código N/A'}</div>
            <div class='boat-progress-display'>Progresso: ${typeof b.progress === 'number' ? Math.round(b.progress) : 0}%</div>
            <div class='overlay-grid' style='margin-top: 6px;'>
              <i class='bi bi-arrows-move overlay-icon move-icon' title='Arrastar'></i>
              <div class='overlay-spacer'></div>
              <i class='bi bi-pencil-square overlay-icon edit-icon' title='Editar'></i>
              <i class='bi bi-clipboard-check overlay-icon docs-icon' title='Ver Documentação'></i>
            </div>
        ...
        // Enable dragging from the slot
        const boatInfoEl = content.querySelector('.boat-info');
        if (boatInfoEl) {
          boatInfoEl.addEventListener('dragstart', function(e) {
            e.dataTransfer.setData('text/plain', this.dataset.boatId);
            slotEl.classList.add('dragging');
          });
          boatInfoEl.addEventListener('dragend', function() {
            slotEl.classList.remove('dragging');
          });
        }

        // Attach edit/docs actions
        const editIcon = content.querySelector('.edit-icon');
        const docsIcon = content.querySelector('.docs-icon');
        if (editIcon) {
          editIcon.addEventListener('click', function(e) {
            e.stopPropagation();
            selectBoat(b.id);
            openEditBoatModal(b.id);
          });
        }
        if (docsIcon) {
          docsIcon.addEventListener('click', function(e) {
    ...
    updateCounters();
  } catch(e) { console.warn('loadBoats warn:', e); }
}
...
function findAvailableSlot(warehouse) {
  ...
  // Sem warehouse: procurar primeiro em Laminação, depois Montagem
  const order = ['laminacao', 'montagem'];
  for (const w of order) {
    const empty = slots.find(s => s.dataset.warehouse === w && !s.dataset.boatId);
    if (empty) { return { warehouse: w, slot: empty.dataset.slot, element: empty }; }
  }
  return null;
}
