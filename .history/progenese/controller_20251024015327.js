...
async function fetchOccupancyFromAPI() {
  try {
    ...
    // Helper: detect temporary IDs created in-memory
    function isTempId(id) {
      return typeof id === 'string' && id.startsWith('tmp_');
    }

    // Robust binder for Save button
    function bindSaveButton() {
      const btn = document.getElementById('saveChangesBtn');
      if (btn && !btn.dataset.bound) {
        btn.addEventListener('click', saveOccupancy);
        btn.dataset.bound = '1';
      }
    }
...
-      if (saveChangesBtn) saveChangesBtn.addEventListener('click', saveOccupancy);
+      bindSaveButton();
  ...
  try {
    ...
    for (const b of newCandidates) {
      try {
        ...
        // Atualizar ID nos arrays correspondentes
