...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Destinatário: exige SMTP_TO_EMAIL_MATERIAL, sem fallback
        $toEmail = env('SMTP_TO_EMAIL_MATERIAL', '');
        $toName = env('SMTP_TO_NAME_MATERIAL', 'Marcelo');
        if (!$toEmail) {
            $response['error'] = 'Destinatário não configurado: SMTP_TO_EMAIL_MATERIAL ausente.';
            write_log($logFile, ['event' => 'material_to_missing', 'ip' => $remoteIp]);
            header('Content-Type: application/json');
            echo json_encode($response);
            exit;
        }
